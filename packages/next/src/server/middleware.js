 ...
 */
export async function getSession(req, options = { cookieName: 'cinaconnect-session' }) {
    const cookieName = options.cookieName;
    let cookieValue = null;
    // Handle NextRequest (edge runtime)
    if ('cookies' in req && typeof req.cookies === 'function') {
        cookieValue = req.cookies().get(cookieName)?.value ?? null;
    }
 ...
 */
function decodeSessionToken(token, _secret) {
    try {
 ...
 */
export async function verifySiweMessage(message, signature, options) {
    const { recoverAddress, hashMessage } = await import('viem');
    // Validate signature format
    if (!signature.startsWith('0x') || signature.length !== 132) {
        throw new Error(`Invalid signature format: expected 0x-prefixed 65-byte hex string, got ${signature.length} chars`);
    }
    ...
    if (recovered.toLowerCase() !== parsed.address.toLowerCase()) {
        throw new Error(`Address mismatch: recovered ${recovered} but message claims ${parsed.address}`);
    }
    // Verify domain if specified
    if (options.domain && parsed.domain !== options.domain) {
        throw new Error(`Domain mismatch: message domain "${parsed.domain}" does not match expected "${options.domain}"`);
    }
 ...
 */
export function withCinaConnectAuth(handler, options) {
    const client = createServerClient({
