...
export interface XrplConnector {
  ...
  connect(params?: { network?: XrplNetwork }): Promise<XrplConnectionResult>;
   ...
   */
  signTransaction(params: { transaction: Record<string, unknown> }): Promise<{ signedTransaction: Record<string, unknown>; txBlob: string }>;

  /**
   * Send XRP to another address.
   */
  sendXRP(params: XrpSendParams): Promise<{ transactionHash: string }>;
   ...
   */
  getBalance(address?: string): Promise<{ balance: string; unit: 'drops' }>;

  /**
   * Update account settings.
   */
  updateAccountSettings(params: AccountSettingsParams): Promise<{ transactionHash: string }>;

  /**
   * Set or remove a trust line.
   */
  setTrustLine(params: TrustLineParams): Promise<{ transactionHash: string }>;

  /**
   * Mint a new NFT on the XRP Ledger.
   */
  mintNFT(params: NftMintParams): Promise<{ nftId: string; transactionHash: string }>;

  /**
   * Burn an NFT on the XRP Ledger.
   */
  burnNFT(params: NftBurnParams): Promise<{ transactionHash: string }>;
