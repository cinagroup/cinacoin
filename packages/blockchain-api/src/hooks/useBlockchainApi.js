 ...
 * ```
 */
export function useBalance(address, chainId) {
 ...
 * ```
 */
export function useTransactionHistory(address, chainId, limit = 20) {
    const client = getSharedClient();
    ...
    const abortRef = useRef(null);
    const fetchHistory = useCallback((append = false) => {
        if (!address) {
            if (!append)
                setTransactions([]);
            return;
        ...
        client
            ...
            .then((result) => {
            if (!controller.signal.aborted) {
                setTransactions(append ? [...transactions, ...result.items] : result.items);
 ...
 * ```
 */
export function useReverseENS(address) {
    const client = getSharedClient();
 ...
 * ```
 */
export function useTokenMetadata(tokenAddress, chainId) {
    const client = getSharedClient();
...
//# sourceMappingURL=useBlockchainApi.js.map