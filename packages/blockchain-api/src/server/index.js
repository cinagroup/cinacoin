 ...
 *   { balances: Balance[] }
 */
export async function POST_balance(req) {
    try {
 ...
 */
export async function POST_history(req) {
    try {
        const body = await parseBody(req);
        if (!body.address) {
            return errorResponse("Missing required field: address");
        ...
        const client = getClient();
        const result = await client.getTransactionHistory(body.address, body.chainId, body.limit ?? 20, body.cursor);
 ...
 *   { name: string | null }
 */
export async function POST_ens_resolve(req) {
    try {
        const body = await parseBody(req);
        const client = getClient();
        if (body.name) {
 ...
 */
export async function POST_token_metadata(req) {
    try {
        const body = await parseBody(req);
