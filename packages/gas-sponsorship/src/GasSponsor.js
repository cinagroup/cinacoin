 ...
 */
async function fetchNativePriceUsd(chainId) {
    const coingeckoIds = {
        1: "ethereum",
        10: "ethereum",
        137: "matic-network",
 ...
 */
function formatBalance(wei, decimals, symbol) {
...
export class GasSponsor {
     ...
     */
    async estimateGas(userOperation, chainId) {
        const verificationGasLimit = BigInt(userOperation.verificationGasLimit ?? 100000);
        const callGasLimit = BigInt(userOperation.callGasLimit ?? 50000);
        const preVerificationGas = BigInt(userOperation.preVerificationGas ?? 30000);
        const maxFeePerGas = BigInt(userOperation.maxFeePerGas ?? (await this.getMaxFeePerGas(chainId)));
        const maxPriorityFeePerGas = BigInt(userOperation.maxPriorityFeePerGas ?? (await this.getGasPrice(chainId)));
     ...
     */
    async sponsorUserOperation(userOperation, config, chainId) {
        if (!config.sponsorGas) {
            throw new Error("Gas sponsorship is disabled (sponsorGas: false)");
        }
        const paymasterAndData = await getPaymasterAndData(userOperation, config.paymasterUrl, chainId);
        ...
        return {
            ...
            paymasterAddress: (config.paymasterAddress ?? "0x0000000000000000000000000000000000000000"),
            paymasterAndData,
            sponsoredGasWei: (BigInt(userOperation.callGasLimit ?? 0n) +
    ...
    async getGasPrice(chainId) {
        try {
            ...
            const defaults = {
                ...
                8453: 1000000n,
                42161: 100000000n,
                11155111: 1000000000n,
            };
            return defaults[chainId] ?? 1000000000n;
        }
    }
    /**
    ...
    async getMaxFeePerGas(chainId) {
        ...
        const defaults = {
            ...
            8453: 2000000n,
            42161: 200000000n,
            11155111: 2000000000n,
        };
        return defaults[chainId] ?? 2000000000n;
    }
    /**
     ...
     */
    async getSponsoredBalance(paymasterAddress, chainId, threshold) {
     ...
     */
    async fundPaymaster(paymasterAddress, _chainId, amount) {
        return {
            to: paymasterAddress,
            value: amount,
            data: "0x",
...
//# sourceMappingURL=GasSponsor.js.map