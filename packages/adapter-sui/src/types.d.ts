...
export interface SuiWalletProvider {
     ...
     */
    signMessage?(message: Uint8Array | string): Promise<{
        message: string;
        signature: string;
    }>;
    /**
    ...
    on(event: 'accountChanged', handler: (account: {
        address: string;
    }) => void): void;
    on(event: 'disconnect', handler: () => void): void;
    /** Unsubscribe from events. */
    off(event: 'accountChanged', handler: (account: {
        address: string;
    }) => void): void;
 ...
 */
export type SuiFeature = 'sui:connect' | 'sui:signTransaction' | 'sui:signAndExecuteTransaction' | 'sui:signMessage' | 'sui:switchNetwork';
/**
