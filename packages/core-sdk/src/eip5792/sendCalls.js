 ...
 */
export async function walletSendCalls(client, params) {
    const account = params.from ?? client.account;
    if (!account) {
 ...
 */
export async function sendSingleCall(client, call, account, chainId, capabilities) {
 ...
 * @returns SendCallsResult with batch ID.
 */
export async function sendErc20Transfer(client, tokenAddress, recipient, amount, account, chainId) {
 ...
 * @returns SendCallsResult with batch ID.
 */
export async function sendBatch(client, calls, account, chainId, capabilities, version) {
    if (calls.length === 0) {
