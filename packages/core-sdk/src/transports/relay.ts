...
export class RelayTransport extends EventEmitter {
      ...
      try {
        ...
        this.ws.onmessage = (event: MessageEvent) => {
          try {
            ...
            console.warn('[OnChainUX] Failed to parse relay message:', event.data);
          }
        };
  ...
  private handleMessage(data: unknown): void {
    ...
    switch (type) {
      ...
      case 'ack':
        ...
        break;
      case 'error':
        console.error('[OnChainUX] Relay error:', msg.message);
  ...
  private reconnect(): void {
    ...
    console.log(
      `[OnChainUX] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.config.maxReconnectAttempts})`,
    );
    setTimeout(() => {
      this.connect().catch(() => {
