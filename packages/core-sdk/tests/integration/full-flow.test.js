...
class MockConnector extends Connector {
    constructor() {
        super(...arguments);
        this.id = 'mock-integration';
        this.name = 'Mock Integration Wallet';
        this.icon = '';
        this.installed = true;
        this.type = 'injected';
    ...
    failOn(method) { this._failOn = method; }
    async connect(params) {
        if (this._failOn === 'connect')
    ...
    async signTransaction(tx) {
        if (this._failOn === 'signTransaction')
            throw new Error('TX signing rejected');
        if (!this._connected)
            throw new Error('Not connected');
