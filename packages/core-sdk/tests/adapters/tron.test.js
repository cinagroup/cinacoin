...
import { TRONChainAdapter, TRON_CHAINS, TRON_WALLETS, isValidTRONAddress, base58ToHex, hexToBase58, } from '../../src/adapters/tron.js';
/* ------------------------------------------------------------------ */
/*  Address Validation                                                 */
/* ------------------------------------------------------------------ */
describe('isValidTRONAddress', () => {
    ...
    it('rejects empty and non-string inputs', () => {
        ...
        expect(isValidTRONAddress(123)).toBe(false);
    });
});
...
describe('TRONChainAdapter buildTransfer', () => {
    ...
    it('builds a valid TRX transfer', () => {
        const tx = adapter.buildTransfer('TN2Y7e5RLkKz6kBPZHMoCjLpQJvGvCqTjZ', '1000000');
        expect(tx.to).toBe('TN2Y7e5RLkKz6kBPZHMoCjLpQJvGvCqTjZ');
        expect(tx.value).toBe('1000000');
    });
    it('throws on invalid recipient', () => {
        expect(() => adapter.buildTransfer('invalid-address', '1000000')).toThrow('Invalid recipient address');
    });
});
describe('TRONChainAdapter buildTRC20Transfer', () => {
    ...
    it('throws on invalid contract address', () => {
        expect(() => adapter.buildTRC20Transfer({
...
describe('TRONChainAdapter sendTransaction', () => {
    ...
    it('throws when no provider', async () => {
        await expect(adapter.sendTransaction({ to: 'TN2Y7e5RLkKz6kBPZHMoCjLpQJvGvCqTjZ', value: '1000' })).rejects.toThrow('No provider connected');
    });
});
/* ------------------------------------------------------------------ */
/*  signMessage                                                        */
...
//# sourceMappingURL=tron.test.js.map