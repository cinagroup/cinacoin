...
import { TONChainAdapter, TON_CHAINS, TON_WALLETS, isValidTONAddress, parseTONAddress, hexToBase64url, base64urlToHex, } from '../../src/adapters/ton.js';
/* ------------------------------------------------------------------ */
/*  Address Validation                                                 */
/* ------------------------------------------------------------------ */
...
describe('TONChainAdapter buildTransfer', () => {
    ...
    it('builds a valid transfer', () => {
        const tx = adapter.buildTransfer('EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N', '1000000000');
    ...
    it('throws on invalid recipient', () => {
        expect(() => adapter.buildTransfer('invalid-address', '1000000000')).toThrow('Invalid recipient address');
    });
});
describe('TONChainAdapter buildJettonTransfer', () => {
    ...
    it('throws on invalid jetton master', () => {
        expect(() => adapter.buildJettonTransfer({
...
describe('TONChainAdapter sendTransaction', () => {
    ...
    it('throws when no provider', async () => {
        await expect(adapter.sendTransaction({ to: 'EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N', value: '1000' })).rejects.toThrow('No provider connected');
    });
});
/* ------------------------------------------------------------------ */
/*  signMessage                                                        */
...
//# sourceMappingURL=ton.test.js.map