...
describe('QRTransport', () => {
    ...
    it('throws on switchChain', async () => {
        ...
        await expect(transport.switchChain(1)).rejects.toThrow('QR transport does not support chain switching');
    });
    it('throws on signMessage', async () => {
        ...
        await expect(transport.signMessage('hello')).rejects.toThrow('Sign via session layer');
    });
    it('throws on signTransaction', async () => {
        ...
        await expect(transport.signTransaction({ from: '0x', to: '0x' })).rejects.toThrow('Sign via session layer');
    });
