...
describe("Scoring functions", () => {
  ...
  it("scoreWallet returns valid score and reasons", () => {
    const rec = scoreWallet(
      sampleWallets[0], // MetaMask
      { targetChain: "evm", platform: "browser", isMobile: false },
      { history: [], successRates: {} },
      DEFAULT_WEIGHTS
    );
  ...
  it("scoreWallet gives zero for incompatible chain", () => {
    const rec = scoreWallet(
      sampleWallets[2], // Phantom (solana only)
      { targetChain: "bitcoin", platform: "browser", isMobile: false },
      { history: [], successRates: {} },
      DEFAULT_WEIGHTS
    );
