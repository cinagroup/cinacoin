 ...
 */
function createSolanaVerifier() {
    return async (input) => {
        try {
            const valid = await verifyEd25519(input.message, input.signature, input.address);
            return {
                chainType: 'solana',
                data: {},
                signature: input.signature,
 ...
 */
function createBitcoinVerifier() {
    return async (input) => {
        try {
            const valid = await verifyBip322(input.message, input.signature, input.address);
            return {
                chainType: 'bitcoin',
                data: {},
                signature: input.signature,
 ...
 */
async function recoverAddressEvm(_message, _signature) {
    // Stub — real implementation uses ethers.verifyMessage or viem.verifyMessage
    throw new Error('recoverAddressEvm: Not implemented in verifier-registry. ' +
 ...
 */
async function verifyEd25519(_message, _signature, _publicKey) {
    // Stub — real implementation uses @noble/ed25519
    throw new Error('verifyEd25519: Not implemented in verifier-registry. ' +
        'Install @cinaconnect/siwx with full crypto dependencies.');
}
/**
 * Verify a BIP-322 Bitcoin signature.
 */
async function verifyBip322(_message, _signature, _address) {
    // Stub — real implementation uses bitcoinjs-lib
    throw new Error('verifyBip322: Not implemented in verifier-registry. ' +
        'Install @cinaconnect/siwx with full crypto dependencies.');
}
/**
...
export class VerifierRegistry {
     ...
     */
    registerVerifier(namespace, verifyFn, options = {}) {
        const existing = this._verifiers.get(namespace);
        if (existing?.source === 'builtin') {
            // Shadowing a built-in — log warning in dev
            if (typeof console !== 'undefined' && process.env?.NODE_ENV !== 'production') {
                console.warn(`[VerifierRegistry] Shadowing built-in verifier for "${namespace}" ` +
     ...
     */
    removeVerifier(namespace) {
        const descriptor = this._verifiers.get(namespace);
        if (!descriptor)
            return false;
        if (descriptor.source === 'builtin') {
            if (typeof console !== 'undefined') {
                console.warn(`[VerifierRegistry] Cannot remove built-in verifier for "${namespace}". ` +
    ...
    _registerBuiltin(namespace, name, verifyFn) {
        this._verifiers.set(namespace, {
            name,
            namespace,
            verify: verifyFn,
