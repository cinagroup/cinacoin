...
export function useCloudAuth(config) {
    ...
    const error = null;
    const createSession = (_params) => auth.createSession(_params);
    const verifySession = () => auth.verifySession();
    const revokeSession = () => auth.revokeSession();
    return { session, createSession, verifySession, revokeSession, isLoading, error };
}
// ---------------------------------------------------------------------------
// useCloudSession
 ...
 * ```
 */
export function useCloudSession(auth) {
    // NOTE: Full React implementation with useState / useEffect / useCallback.
    //
    // Implementation sketch:
    //
 ...
 * ```
 */
export function useCloudAuthEvents(auth, onEvent) {
...
//# sourceMappingURL=cloud-hooks.js.map