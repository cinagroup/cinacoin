 ...
 */
export function createBitcoinSignInMessage(params) {
 ...
 */
export async function verifyBitcoinSignature(input) {
    try {
        let isValid;
        if (typeof globalThis.__bitcoinVerify === 'function') {
            // Custom verify hook provided by the runtime
            isValid = globalThis.__bitcoinVerify(input.message, input.signature, input.address);
        }
        else {
 ...
 */
function validateBitcoinSignatureFormat(signature, address) {
    if (!signature)
        return false;
 ...
 */
export function parseBitcoinMessage(message) {
