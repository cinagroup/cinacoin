 ...
 */
export function createSolanaSignInMessage(params) {
 ...
 */
export async function verifySolanaSignature(input) {
    try {
        ...
        if (typeof globalThis.__solanaVerify === 'function') {
            ...
            const signatureBytes = typeof input.signature === 'string'
                ...
                : input.signature;
            isValid = globalThis.__solanaVerify(signatureBytes, messageBytes, input.address);
        }
        else {
 ...
 */
function validateSolanaSignatureFormat(signature) {
 ...
 */
export function parseSolanaMessage(message) {
