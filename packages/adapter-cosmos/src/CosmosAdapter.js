...
export class CosmosAdapter {
    constructor(config) {
        ...
        this.id = 'cosmos-adapter';
        /** Human-readable adapter name. */
        this.name = 'Cosmos Chain Adapter';
        this._connector = null;
        this._address = null;
        this._chains = [];
        this._preferredWallet = 'keplr';
        ...
        if (!this._rpcUrl) {
            throw new Error(`No RPC URL configured for chain "${config.chainId}". Provide rpcUrl in config or use a known chain ID.`);
        }
    }
    /* ---- Configuration ---- */
    ...
    async connect(walletId) {
        ...
        if (!connector) {
            throw new Error('No Cosmos wallet found. Install Keplr (https://keplr.app) or Leap (https://leapwallet.io).');
        }
    ...
    async getBalance(address) {
        ...
        if (this._restUrl) {
            try {
                const response = await fetch(`${this._restUrl}/cosmos/bank/v1beta1/balances/${address}`);
                if (response.ok) {
    ...
    async getBalanceFormatted(address, denom) {
        ...
        const target = balances.find((c) => c.denom === (denom ?? this._getNativeDenom()));
        if (!target)
            return '0';
    ...
    async sendTransfer(params) {
        ...
        try {
            const txHash = await this._connector.sendTransfer(this._chainId, params.to, amountStr, denom, params.memo);
    ...
    async signTransaction(tx) {
        ...
        throw new Error('signTransaction expects a Cosmos SignDoc. Use sendTransfer() for simple transfers.');
    }
    /**
    ...
    async getAccountInfo(address) {
        if (this._restUrl) {
            try {
                const response = await fetch(`${this._restUrl}/cosmos/auth/v1beta1/accounts/${address}`);
                if (response.ok) {
                    const data = await response.json();
    ...
    async _queryBalanceViaRPC(address) {
        const response = await fetch(`${this._rpcUrl}/abci_query`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
//# sourceMappingURL=CosmosAdapter.js.map