 ...
 * <script>
 *   OnChainUX.renderConnectButton('#my-button', {
...
async function handleConnect(btn: HTMLElement, config: OnChainUXConfig & ConnectButtonOptions): Promise<void> {
  _state = "connecting";
  ...
  try {
    ...
    btn.style.opacity = "1";
    btn.style.pointerEvents = "auto";
    console.error("[OnChainUX] Connection failed:", err);
  }
}

function handleDisconnect(btn: HTMLElement, config: OnChainUXConfig & ConnectButtonOptions): void {
  _state = "disconnected";
  _address = null;
  btn.textContent = config.label ?? "Connect Wallet";
