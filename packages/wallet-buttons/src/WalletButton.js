...
export const WalletButton = ({ walletId, variant = 'default', size = 'md', onClick, className = '', isLoading = false, disabled = false, label, showConnectedBadge = true, }) => {
    ...
    return (_jsxs("button", { type: "button", className: `cc-wallet-button cc-wallet-button--${variant} cc-wallet-button--${size} ${className}`, style: baseStyle, onClick: handleClick, disabled: disabled || isLoading, "aria-label": `Connect with ${label ?? wallet.name}`, title: `Connect with ${wallet.name}`, onMouseEnter: (e) => {
            ...
            el.style.borderColor = isMinimal ? 'none' : '1px solid var(--cc-wb-border, #e2e8f0)';
        }, children: [isLoading ? (_jsx("span", { className: "cc-wallet-button__spinner", style: {
                    width: sizes.icon,
                    height: sizes.icon,
                    ...
                    display: 'block',
                } })) : (_jsx("img", { src: wallet.icon, alt: "", style: imgStyle, loading: "lazy", onError: (e) => {
                    e.target.style.display = 'none';
                } })), !isRound && !isMinimal && (_jsx("span", { children: label ?? wallet.name })), !isRound && isMinimal && (_jsx("span", { style: { textDecoration: 'underline', textUnderlineOffset: '3px' }, children: label ?? wallet.name })), showConnectedBadge && connected && !isLoading && (_jsx("span", { className: "cc-wallet-button__badge", style: {
                    position: 'absolute',
                    top: isRound ? '-2px' : '-4px',
