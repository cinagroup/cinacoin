...
export function useWalletButtons(
  ...
  const getWalletButtonData = useCallback(
    (walletId: string): WalletButtonData | null => resolveWalletData(walletId),
    [],
  );

  const buttons = useMemo<WalletButtonData[]>(
    () =>
      WALLET_REGISTRY.map((w) => getWalletButtonData(w.walletId))
        .filter((b): b is WalletButtonData => b !== null),
    [getWalletButtonData],
  );

  const isConnected = useCallback(
    (walletId: string): boolean => connectedWallets.has(walletId),
    [connectedWallets],
  );
