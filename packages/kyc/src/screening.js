 ...
 */
import { isSanctioned, isMixer, isScamAddress, isRiskyExchange, getMatchedLists, listBasedRiskLevel, } from './lists.js';
/* ── helpers ───────────────────────────────────────────────────── */
function now() {
 ...
 */
function listScore(address) {
    if (isSanctioned(address))
        return 100;
    if (isMixer(address))
        return 85;
    if (isScamAddress(address))
        return 90;
    if (isRiskyExchange(address))
        return 60;
    return 5;
}
/**
 ...
 */
export function screenAddress(address) {
 ...
 */
export function screenTransaction(tx) {
    ...
    const compositeScore = Math.min(100, Math.max(sender.riskScore, recipient.riskScore) + patterns.score);
    const riskLevel = compositeScore >= 100
        ? 'sanctioned'
        : compositeScore >= 75
            ? 'high'
            : compositeScore >= 40
    ...
    return {
        ...
        recommendation: riskLevel === 'sanctioned'
...
export function getComplianceReport(address) {
    ...
    return {
        ...
        sanctionsHistory: screening.isSanctioned
            ...
            : [],
        recommendation: screening.isSanctioned
            ? 'block'
