...
export type KycStatus =
  | 'verified'       // Fully verified, passed all checks
  | 'pending'        // Documents submitted, awaiting review
  | 'unverified'     // No KYC performed yet
  | 'flagged'        // Failed review or flagged for manual review
  | 'rejected';      // Explicitly rejected, may not transact

/** Risk level assigned to an address or transaction. */
export type RiskLevel =
  | 'low'        // No concerns
  | 'medium'     // Some risk factors present
  | 'high'       // Significant risk, enhanced due diligence recommended
  | 'sanctioned'; // On OFAC / sanctions list — block immediately
...
export interface ComplianceReport {
  ...
  sanctionsHistory: Array<{ list: string; addedAt: string; removedAt?: string }>;
  /** Recommended action. */
