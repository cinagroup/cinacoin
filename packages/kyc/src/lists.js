...
const ofacSdnSet = new Set();
/** Known scam / fraud addresses. */
const scamAddressSet = new Set();
/** Mixer / tumbler addresses. */
const mixerAddressSet = new Set();
/** High-risk exchange addresses. */
const riskyExchangeSet = new Set();
...
export function seedLists() {
    ...
    for (const a of SEED_OFAC)
        ofacSdnSet.add(normalize(a));
    for (const a of SEED_MIXERS)
        mixerAddressSet.add(normalize(a));
    for (const a of SEED_SCAMS)
        scamAddressSet.add(normalize(a));
    for (const a of SEED_RISKY_EXCHANGES)
        riskyExchangeSet.add(normalize(a));
}
/* ── helpers ───────────────────────────────────────────────────── */
 ...
 */
export function isRiskyExchange(address) {
    return riskyExchangeSet.has(normalize(address));
}
/**
 * Return the list names that the address matches.
 */
export function getMatchedLists(address) {
    const n = normalize(address);
    const lists = [];
    if (ofacSdnSet.has(n))
        lists.push('OFAC SDN');
    if (mixerAddressSet.has(n))
        lists.push('Mixer / Tumbler');
    if (scamAddressSet.has(n))
        lists.push('Known Scam');
    if (riskyExchangeSet.has(n))
        lists.push('High-Risk Exchange');
    return lists;
}
/**
 ...
 */
export function listBasedRiskLevel(address) {
    const n = normalize(address);
    if (ofacSdnSet.has(n))
        return 'sanctioned';
    if (mixerAddressSet.has(n))
...
export function updateLists(update) {
    if (update.ofacSdn) {
        ofacSdnSet.clear();
        for (const a of update.ofacSdn)
            ofacSdnSet.add(normalize(a));
    }
    if (update.mixers) {
        mixerAddressSet.clear();
        for (const a of update.mixers)
            mixerAddressSet.add(normalize(a));
    }
    if (update.scams) {
        scamAddressSet.clear();
        for (const a of update.scams)
            scamAddressSet.add(normalize(a));
    }
    if (update.riskyExchanges) {
        riskyExchangeSet.clear();
        for (const a of update.riskyExchanges)
            riskyExchangeSet.add(normalize(a));
    }
}
// Auto-seed on import
