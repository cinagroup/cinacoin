 ...
 */
export function buildMagicLink(params, token) {
 ...
 */
export function validateMagicLinkToken(token, email, storedToken, expiry) {
    // Check expiration
 ...
 */
export function validateOTP(provided, stored, expiry) {
    // Check expiration
    if (Date.now() / 1000 >= expiry) {
        return false;
    }
 ...
 */
export async function loginWithEmail(email, sendEmail, deriveWallet, generateJWT, loginMethod = 'magiclink', verifyData) {
    // Send step
    if (!verifyData) {
        if (loginMethod === 'magiclink') {
            ...
            await sendEmail(email, 'Sign in to your account', `Click this link to sign in: ${link}\n\nThis link expires in 1 hour.`);
            throw new Error('AUTH_PENDING'); // Signal that auth is pending
        }
        else {
            const otp = generateOTP();
            const expiry = Math.floor(Date.now() / 1000) + 300;
            await sendEmail(email, 'Your verification code', `Your code is: ${otp}\n\nThis code expires in 5 minutes.`);
            throw new Error('AUTH_PENDING'); // Signal that auth is pending
        }
    }
    // Verification step
    ...
    if (loginMethod === 'magiclink') {
        ...
        isValid = validateMagicLinkToken(verifyData.token, email, verifyData.storedToken, verifyData.expiry);
    }
    else {
