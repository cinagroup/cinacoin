...
import { buildAppleAuthUrl, decodeAppleIdToken, } from '../../src/providers/apple.js';
...
describe('Apple Provider - decodeAppleIdToken', () => {
    ...
    it('should throw on invalid JWT (wrong number of parts)', () => {
        expect(() => decodeAppleIdToken('not-a-jwt')).toThrow('Invalid Apple ID token: not a valid JWT');
        expect(() => decodeAppleIdToken('header.payload')).toThrow('Invalid Apple ID token: not a valid JWT');
    });
