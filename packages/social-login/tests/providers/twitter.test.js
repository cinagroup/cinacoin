...
import { buildTwitterAuthUrl, fetchTwitterUserProfile, } from '../../src/providers/twitter.js';
...
describe('Twitter Provider - fetchTwitterUserProfile', () => {
    it('should fetch user profile with bearer token', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(expect.stringContaining('https://api.twitter.com/2/users/me'), expect.objectContaining({
    ...
    it('should throw on error response', async () => {
        ...
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockResponse));
        await expect(fetchTwitterUserProfile('bad-token')).rejects.toThrow('Twitter userinfo fetch failed');
        vi.unstubAllGlobals();
    });
});
