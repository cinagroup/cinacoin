...
describe('validateMagicLinkToken', () => {
    it('should validate a matching token', () => {
        const valid = validateMagicLinkToken('abc123', 'user@example.com', 'abc123', Math.floor(Date.now() / 1000) + 3600);
        expect(valid).toBe(true);
    });
    it('should reject non-matching token', () => {
        const valid = validateMagicLinkToken('abc123', 'user@example.com', 'xyz789', Math.floor(Date.now() / 1000) + 3600);
        expect(valid).toBe(false);
    });
    it('should reject expired token', () => {
        const valid = validateMagicLinkToken('abc123', 'user@example.com', 'abc123', Math.floor(Date.now() / 1000) - 100);
        expect(valid).toBe(false);
    });
});
describe('validateOTP', () => {
    it('should validate a matching OTP', () => {
        const valid = validateOTP('123456', '123456', Math.floor(Date.now() / 1000) + 300);
        expect(valid).toBe(true);
    });
    it('should reject non-matching OTP', () => {
        const valid = validateOTP('123456', '654321', Math.floor(Date.now() / 1000) + 300);
        expect(valid).toBe(false);
    });
    it('should reject expired OTP', () => {
        const valid = validateOTP('123456', '123456', Math.floor(Date.now() / 1000) - 100);
        expect(valid).toBe(false);
    });
});
