...
import { buildGoogleAuthUrl, exchangeCodeForTokens, fetchGoogleUserProfile, loginWithGoogle, } from '../../src/providers/google.js';
import { loginWithEmail, generateOTP, generateMagicLinkToken, buildMagicLink, validateMagicLinkToken, validateOTP, } from '../../src/providers/email.js';
...
describe('Google Provider', () => {
    ...
    describe('exchangeCodeForTokens', () => {
        it('should make a POST request to Google token endpoint', async () => {
            ...
            const result = await exchangeCodeForTokens('auth-code-123', { clientId: 'test-client-id', redirectUri: 'https://example.com/callback' }, 'client-secret');
        ...
        it('should throw on error response', async () => {
            ...
            vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockResponse));
            await expect(exchangeCodeForTokens('bad-code', { clientId: 'test', redirectUri: 'https://example.com' }, 'secret')).rejects.toThrow('Google token exchange failed');
            vi.unstubAllGlobals();
        });
    });
    describe('fetchGoogleUserProfile', () => {
        it('should fetch user profile with bearer token', async () => {
            ...
            expect(fetch).toHaveBeenCalledWith('https://www.googleapis.com/oauth2/v3/userinfo', expect.objectContaining({
        ...
        it('should throw on error response', async () => {
            ...
            vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockResponse));
            await expect(fetchGoogleUserProfile('bad-token')).rejects.toThrow('Google userinfo fetch failed');
            vi.unstubAllGlobals();
        });
    });
    describe('loginWithGoogle', () => {
        ...
        it('should throw when profile has no sub', async () => {
            ...
            await expect(loginWithGoogle('auth-code', { clientId: 'test', redirectUri: 'https://example.com', clientSecret: 'secret' }, async () => ({ address: '0x0' }))).rejects.toThrow('no user ID in profile');
            vi.unstubAllGlobals();
        });
    });
});
...
describe('Email Provider', () => {
    ...
    describe('validateMagicLinkToken', () => {
        it('should return true for valid matching token', () => {
            ...
            const result = validateMagicLinkToken('abc123', 'user@example.com', 'abc123', expiry);
            expect(result).toBe(true);
        });
        it('should return false for non-matching token', () => {
            const expiry = Math.floor(Date.now() / 1000) + 3600;
            const result = validateMagicLinkToken('abc123', 'user@example.com', 'wrong-token', expiry);
        ...
        it('should return false for different length tokens', () => {
            const expiry = Math.floor(Date.now() / 1000) + 3600;
            const result = validateMagicLinkToken('abc', 'user@example.com', 'abcdef123', expiry);
            expect(result).toBe(false);
        });
    });
    ...
    describe('loginWithEmail', () => {
        it('should throw AUTH_PENDING for magiclink method without verifyData', async () => {
            ...
            await expect(loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'magiclink')).rejects.toThrow('AUTH_PENDING');
        ...
        it('should throw AUTH_PENDING for otp method without verifyData', async () => {
            ...
            const deriveWallet = vi.fn().mockResolvedValue({ address: '0x0' });
            const generateJWT = vi.fn().mockResolvedValue({ token: 'jwt', expiresAt: 0 });
            await expect(loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'otp')).rejects.toThrow('AUTH_PENDING');
        });
        it('should complete login on valid magiclink verification', async () => {
            ...
            const expiry = Math.floor(Date.now() / 1000) + 3600;
            const result = await loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'magiclink', { token: 'stored-token-value', storedToken, expiry });
        ...
        it('should complete login on valid OTP verification', async () => {
            ...
            const expiry = Math.floor(Date.now() / 1000) + 300;
            const result = await loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'otp', { otp: '123456', storedOTP, expiry });
            expect(result.provider).toBe('email');
        ...
        it('should throw on invalid magiclink token verification', async () => {
            ...
            await expect(loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'magiclink', { token: 'wrong', storedToken: 'right', expiry: Math.floor(Date.now() / 1000) + 3600 })).rejects.toThrow('Invalid or expired verification code');
        });
        it('should throw on expired token verification', async () => {
            const sendEmail = vi.fn();
            const deriveWallet = vi.fn();
            const generateJWT = vi.fn();
            await expect(loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'magiclink', { token: 'token', storedToken: 'token', expiry: Math.floor(Date.now() / 1000) - 100 })).rejects.toThrow('Invalid or expired verification code');
        });
        it('should throw on missing verification data', async () => {
            const sendEmail = vi.fn();
            const deriveWallet = vi.fn();
            const generateJWT = vi.fn();
            await expect(loginWithEmail('user@example.com', sendEmail, deriveWallet, generateJWT, 'magiclink', {})).rejects.toThrow('Invalid verification data');
        });
    });
});
