...
import { StateSync, InMemoryStorage, syncEvmState, syncSolanaState, syncBitcoinState, } from "../src/index.js.js";
describe("StateSync", () => {
    ...
    it("throws when syncing unregistered chain", async () => {
        const storage = new InMemoryStorage();
        const sync = new StateSync(storage);
        await expect(sync.syncChain("solana")).rejects.toThrow("No adapter registered");
    });
