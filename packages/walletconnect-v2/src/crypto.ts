 ...
 */
export function encodeType0Envelope(
  symKey: string,
  payload: unknown,
): string {
 ...
 */
export function decodeType0Envelope(
  symKey: string,
  envelopeJson: string,
): unknown {
 ...
 */
export function encodeType1Envelope(
 ...
 * @returns Decrypted JSON object.
 */
export function decodeType1Envelope(
  keypair: X25519Keypair,
  envelopeJson: string,
): unknown {
  const envelope = JSON.parse(envelopeJson) as {
  ...
  const senderPubBytes = Uint8Array.from(
    atob(envelope.senderPublicKey),
    (c) => c.charCodeAt(0),
  );
 ...
 */
export function isValidSymKey(symKey: string): boolean {
