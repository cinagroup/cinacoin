 ...
 */
export function getDefaultRequiredNamespaces(config) {
 ...
 */
export function buildSendTransaction(tx) {
    return { method: 'eth_sendTransaction', params: [tx] };
}
/**
 * Build a personal_sign request.
 */
export function buildPersonalSign(message, address) {
 ...
 */
export function buildSignTypedDataV4(address, typedData) {
    return { method: 'eth_signTypedData_v4', params: [address, typedData] };
}
/**
 * Build an eth_sign request.
 */
export function buildEthSign(address, message) {
    const hexMessage = message.startsWith('0x')
        ? message
        : '0x' + Array.from(new TextEncoder().encode(message))
            .map((b) => b.toString(16).padStart(2, '0'))
 ...
 */
export function buildSolanaSignMessage(pubkey, message) {
    return { method: 'solana_signMessage', params: { pubkey, message } };
}
/**
 * Build a solana_signTransaction request.
 */
export function buildSolanaSignTransaction(transaction) {
 ...
 */
export function isEvmMethod(method) {
    return WC_METHODS.includes(method);
}
/**
 * Check if a method is a Solana method.
 */
export function isSolanaMethod(method) {
    return SOLANA_METHODS.includes(method);
}
/**
 * Check if a method is a WC-internal method.
 */
export function isWcInternalMethod(method) {
