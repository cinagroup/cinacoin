...
import { initCinaConnect, resetCinaConnect, open, close, switchChain, isConnected, address, balance, chainId, status, error, isConnecting, hasError, chains, } from './stores.js';
 ...
 * ```
 */
export function createCinaConnect(options = {}) {
    ...
    if (!connector) {
        throw new Error('[CinaConnect] createCinaConnect requires either `connector` or `createConnector` option.');
    }
 ...
 */
export function getCinaConnectContext(contextKey = DEFAULT_CONTEXT_KEY) {
    try {
...
//# sourceMappingURL=createCinaConnect.js.map