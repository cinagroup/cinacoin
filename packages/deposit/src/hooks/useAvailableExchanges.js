...
export function useAvailableExchanges(options = {}) {
    ...
    const exchanges = useMemo(() => {
        return allExchanges.filter((ex) => {
            ...
            if (address) {
                const isSolanaAddress = !address.startsWith("0x") && address.length >= 32;
                if (isSolanaAddress && !ex.supportedNetworks.includes("solana")) {
                    return false;
                }
                const isEvmAddress = address.startsWith("0x");
                if (isEvmAddress &&
                    !ex.supportedNetworks.some((n) => n !== "solana")) {
                    return false;
                }
            }
            return true;
    ...
    const isAvailable = useMemo(() => {
        ...
        return (exchangeId) => availableIds.has(exchangeId);
    }, [exchanges]);
    return { exchanges, isAvailable };
}
//# sourceMappingURL=useAvailableExchanges.js.map