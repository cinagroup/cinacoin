...
import { ConnectButton } from '../src/components/connect-button.js';
import { ConnectModal } from '../src/components/connect-modal.js';
import { WalletCard } from '../src/components/wallet-card.js';
import { ChainSwitcher } from '../src/components/chain-switcher.js';

async function measureRender(template: ReturnType<typeof html>, container: HTMLElement): Promise<number> {
...
describe('Render Benchmarks', () => {
  ...
  it('should render ConnectButton within 5ms', async () => {
    const duration = await measureRender(
      html`<ocx-connect-button></ocx-connect-button>`,
      container,
    );
  ...
  it('should re-render ConnectButton on state change within 3ms', async () => {
    ...
    const btn = container.querySelector('ocx-connect-button') as ConnectButton;
