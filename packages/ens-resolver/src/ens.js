...
import { mainnet, arbitrum, optimism, polygon, base, sepolia, } from "viem/chains";
import { ENS_CHAIN_CONFIG, ENS_ERRORS, ENSResolverError, } from "./types.js.js";
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
const SUPPORTED_CHAIN_IDS = [1, 11155111, 10, 137, 8453, 42161];
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function makeReverseNode(address) {
...
function resolveChainId(chainId, defaultChainId) {
    const cid = chainId ?? defaultChainId;
    return SUPPORTED_CHAIN_IDS.includes(cid) ? cid : defaultChainId;
}
function getChainConfig(chainId) {
    return ENS_CHAIN_CONFIG[chainId];
}
// ---------------------------------------------------------------------------
// ENSResolver class
// ---------------------------------------------------------------------------
export class ENSResolver {
    ...
    async getResolverForNode(node, chainId) {
        const cid = resolveChainId(chainId, this.config.chainId);
        ...
        if (!resolverAddress || isZeroAddress(resolverAddress)) {
            return null;
        }
     ...
     */
    async resolveName(name, chainId) {
        if (!isValidENSName(name)) {
            throw new ENSResolverError(ENS_ERRORS.INVALID_NAME, `Invalid ENS name: ${name}`);
        }
        ...
        try {
            ...
            throw new ENSResolverError(ENS_ERRORS.RESOLVE_FAILED, `Failed to resolve name: ${name}`, err);
        }
    }
    /**
     * Perform a reverse lookup: address → ENS name.
     */
    async reverseLookup(address, chainId) {
        if (!isValidAddress(address)) {
            throw new ENSResolverError(ENS_ERRORS.INVALID_ADDRESS, `Invalid Ethereum address: ${address}`);
        }
        ...
        try {
            ...
            throw new ENSResolverError(ENS_ERRORS.LOOKUP_FAILED, `Failed to reverse lookup: ${address}`, err);
        }
    }
    /**
     * Get the avatar (profile image) URL for an ENS name.
     */
    async getAvatar(nameOrAddress, chainId) {
        let node;
        ...
        try {
            ...
            throw new ENSResolverError(ENS_ERRORS.AVATAR_NOT_FOUND, `Failed to get avatar for: ${nameOrAddress}`, err);
        }
    }
    /**
     * Get a text record value for an ENS name.
     */
    async getText(name, key, chainId) {
        if (!isValidENSName(name)) {
            throw new ENSResolverError(ENS_ERRORS.INVALID_NAME, `Invalid ENS name: ${name}`);
        }
        ...
        try {
            ...
            throw new ENSResolverError(ENS_ERRORS.RESOLVE_FAILED, `Failed to get text record "${key}" for: ${name}`, err);
        }
    }
    /**
     * Get the full ENS profile for a name or address.
     */
    async getProfile(nameOrAddress, chainId) {
        const profile = {
            address: null,
            name: null,
            avatar: null,
        ...
        if (ensName) {
            const [avatar, url, description, email, github, twitter, discord] = await Promise.allSettled([
     ...
     */
    async resolveWithRecords(name, keys, chainId) {
        const address = await this.resolveName(name, chainId);
        const records = {};
        const results = await Promise.allSettled(keys.map((key) => this.getText(name, key, chainId)));
        keys.forEach((key, i) => {
...
// ---------------------------------------------------------------------------
export function createENSResolver(config) {
    return new ENSResolver(config);
}
export async function resolveENSName(name, rpcUrl, chainId) {
    const resolver = new ENSResolver({ rpcUrl, chainId });
    return resolver.resolveName(name, chainId);
}
export async function reverseLookupENS(address, rpcUrl, chainId) {
    const resolver = new ENSResolver({ rpcUrl, chainId });
    return resolver.reverseLookup(address, chainId);
}
export async function getAvatarENS(nameOrAddress, rpcUrl, chainId) {
    const resolver = new ENSResolver({ rpcUrl, chainId });
