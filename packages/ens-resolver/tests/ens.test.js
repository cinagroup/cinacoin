...
import { ENSResolver, createENSResolver, resolveENSName, reverseLookupENS, getAvatarENS, } from "../src/ens.js.js";
import { ENS_CHAIN_CONFIG, ENS_ERRORS, ENSResolverError, } from "../src/types.js.js";
// ---------------------------------------------------------------------------
// Mock viem
// ---------------------------------------------------------------------------
vi.mock("viem", () => ({
    ...
    namehash: vi.fn((name) => {
        ...
        return `0x${Buffer.from(name).toString("hex").padEnd(64, "0")}`;
    }),
    keccak256: vi.fn((data) => `0x${Buffer.from(data).toString("hex").padEnd(64, "0")}`),
    encodePacked: vi.fn((types, values) => `0x${values.join("")}`),
    stringToHex: vi.fn((s) => `0x${Buffer.from(s).toString("hex")}`),
    slice: vi.fn(),
}));
vi.mock("viem/chains", () => ({
...
function mockReadContractFn(fn) {
    createPublicClient.mockReturnValue({
...
describe("ENSResolver", () => {
    ...
    it("resolveName throws on invalid name", async () => {
        await expect(resolver.resolveName("not-a-valid-name")).rejects.toThrow(ENSResolverError);
    });
    // 5. resolveName — rejects name without TLD
    it("resolveName throws on name without TLD", async () => {
        await expect(resolver.resolveName("vitalik")).rejects.toThrow("Invalid ENS name");
    });
    ...
    it("getText throws on invalid name", async () => {
        await expect(resolver.getText("badname", "url")).rejects.toThrow("Invalid ENS name");
    });
    ...
    it("getAvatar resolves by address", async () => {
        mockReadContractSeq("0x0000000000000000000000000000000000000000" // no reverse resolver → returns null
        );
        const result = await resolver.getAvatar("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045");
        expect(result).toBeNull();
    });
    ...
    it("getProfile works with address input", async () => {
        mockReadContract("0x0000000000000000000000000000000000000000");
        const profile = await resolver.getProfile("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045");
        expect(profile.address).toBe("0xd8da6bf26964af9d7eed9e03e53415d37aa96045");
    ...
    it("ENSResolverError has code and message", () => {
        const err = new ENSResolverError(ENS_ERRORS.RESOLVE_FAILED, "test error", { extra: "data" });
    ...
    it("type exports are available", () => {
        const record = { key: "test", value: "val" };
        ...
        const cache = { value: "test", expiresAt: 123 };
    ...
    it("resolveName throws ENSResolverError with RESOLVE_FAILED code on RPC error", async () => {
        ...
        await expect(resolver.resolveName("test.eth")).rejects.toSatisfy((err) => {
    ...
    it("reverseLookup throws LOOKUP_FAILED on RPC error", async () => {
        ...
        await expect(resolver.reverseLookup("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045")).rejects.toSatisfy((err) => {
            if (err instanceof ENSResolverError) {
    ...
    it("getAvatar throws AVATAR_NOT_FOUND on error", async () => {
        ...
        await expect(resolver.getAvatar("test.eth")).rejects.toSatisfy((err) => {
            if (err instanceof ENSResolverError) {
    ...
    it("getText throws RESOLVE_FAILED on error", async () => {
        ...
        await expect(resolver.getText("test.eth", "url")).rejects.toSatisfy((err) => {
            if (err instanceof ENSResolverError) {
                return err.code === ENS_ERRORS.RESOLVE_FAILED;
            }
            return false;
    ...
    it("getProfile returns empty profile for unknown address", async () => {
        mockReadContract("0x0000000000000000000000000000000000000000");
        const profile = await resolver.getProfile("0x1111111111111111111111111111111111111111");
    ...
    it("resolveWithRecords returns address and populated records", async () => {
        ...
        createPublicClient.mockReturnValue({ readContract: readContractFn });
        const result = await resolver.resolveWithRecords("test.eth", ["url", "email"]);
        expect(result.address).toBe("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045");
