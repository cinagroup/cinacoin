      ...
      <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
        ...
        <div v-else-if="status === 'error'" class="py-6 text-center">
          <slot name="error">
            ...
            <button class="mt-3 rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-600" @click="closeModal">Close</button>
          </slot>
        </div>
...
async function handleSelect(connectorId: string) {
  ...
  try {
    ...
    if (connector) {
      emit('wallet-select', connector)
      emit('connected', connector)
    }
    closeModal()
  } catch {
    // status will update to 'error' via provider
  } finally {
