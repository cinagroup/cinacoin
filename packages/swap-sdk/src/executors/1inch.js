...
// OneInchExecutor
// ============================================================
export class OneInchExecutor {
    constructor(apiKey, version = "6.0") {
        this.name = "1inch";
        this.apiKey = apiKey;
    ...
    async isAvailable() {
        try {
            const res = await fetch(`${ONEINCH_API_BASE}/v${this.apiVersion}/56/healthcheck`, {
    ...
    async getQuote(params) {
        ...
        const url = new URL(`${ONEINCH_API_BASE}/v${this.apiVersion}/${chainId}/quote`);
        ...
        const route = data.protocols.map((p) => ({
            protocol: p.name,
    ...
    async getTransaction(quote, slippageBps) {
        ...
        const url = new URL(`${ONEINCH_API_BASE}/v${this.apiVersion}/${chainId}/swap`);
    ...
    async getSupportedTokens(chainId) {
        const res = await fetch(`${ONEINCH_TOKENS_BASE}/v1.2/${chainId}/tokens`, {
            headers: { Authorization: `Bearer ${this.apiKey}` },
        });
        if (!res.ok)
            return [];
        const data = await res.json();
        return Object.values(data.tokens).map((t) => ({
            address: t.address,
            symbol: t.symbol,
            name: t.name,
            decimals: t.decimals,
    ...
    fromApiAddress(addr) {
        ...
        return addr.toLowerCase() === native.toLowerCase() ? "native" : addr;
    }
    getChainId(quote) {
