...
describe('SocialRecoveryManager - Initialization', () => {
    ...
    it('should initialize with default 3-of-5 threshold', () => {
        ...
        expect(config.recoveryDelay).toBe(86400);
    });
    ...
    it('should throw when threshold exceeds guardian count', () => {
        ...
        expect(() => manager.initialize(guardians, 5)).toThrow('Threshold (5) exceeds guardian count (3)');
    });
...
describe('SocialRecoveryManager - Guardian Management', () => {
    ...
    it('should throw when adding duplicate guardian', () => {
        ...
        expect(() => manager.addGuardian(guardians[0].address, 'Duplicate')).toThrow('Guardian already exists');
    });
    it('should throw when adding without initialization', () => {
        const freshManager = new SocialRecoveryManager();
        expect(() => freshManager.addGuardian('0xabc', 'Test')).toThrow('Recovery not initialized');
    });
    ...
    it('should throw when removing unknown guardian', () => {
        expect(() => manager.removeGuardian('0xunknown')).toThrow('Guardian not found or already inactive');
    });
...
describe('SocialRecoveryManager - Recovery Flow', () => {
    ...
    it('should throw when initiating recovery without initialization', () => {
        const freshManager = new SocialRecoveryManager();
        expect(() => freshManager.initiateRecovery(account, newOwner)).toThrow('Recovery not initialized');
    });
    it('should reject duplicate pending recovery requests', () => {
        manager.initiateRecovery(account, newOwner);
        expect(() => manager.initiateRecovery(account, newOwner)).toThrow('Recovery request already pending');
    });
    it('should accept guardian signatures and track count', () => {
        const request = manager.initiateRecovery(account, newOwner);
        const guardians = manager.getActiveGuardians();
        const result1 = manager.addGuardianSignature(request.id, guardians[0].address, '0xsignature1');
        expect(result1.count).toBe(1);
        expect(result1.thresholdReached).toBe(false);
        const result2 = manager.addGuardianSignature(request.id, guardians[1].address, '0xsignature2');
        expect(result2.count).toBe(2);
        expect(result2.thresholdReached).toBe(false);
        const result3 = manager.addGuardianSignature(request.id, guardians[2].address, '0xsignature3');
    ...
    it('should reject signature from inactive guardian', () => {
        ...
        const guardians = manager.getActiveGuardians();
        manager.removeGuardian(guardians[0].address);
        expect(() => manager.addGuardianSignature(request.id, guardians[0].address, '0xsig')).toThrow('Not an active guardian');
    });
    it('should reject duplicate signatures from same guardian', () => {
        ...
        expect(() => manager.addGuardianSignature(request.id, guardians[0].address, '0xsig2')).toThrow('Guardian already signed');
    });
    it('should reject signature on executed recovery', () => {
        const request = manager.initiateRecovery(account, newOwner);
        request.executed = true;
        const guardians = manager.getActiveGuardians();
        expect(() => manager.addGuardianSignature(request.id, guardians[0].address, '0xsig')).toThrow('Recovery request already executed');
    });
    it('should reject signature on cancelled recovery', () => {
        const request = manager.initiateRecovery(account, newOwner);
        request.cancelled = true;
        const guardians = manager.getActiveGuardians();
        expect(() => manager.addGuardianSignature(request.id, guardians[0].address, '0xsig')).toThrow('Recovery request was cancelled');
    });
    it('should reject signature for nonexistent request', () => {
        const guardians = manager.getActiveGuardians();
        expect(() => manager.addGuardianSignature('nonexistent', guardians[0].address, '0xsig')).toThrow('Recovery request not found');
    });
...
//# sourceMappingURL=social-recovery.test.js.map