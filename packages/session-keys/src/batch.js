...
export class BatchTransactionBuilder {
     ...
     * @returns This builder for chaining
     */
    addApprove(token, spender, amount) {
    ...
    build(sessionKey, policy) {
        ...
        for (let i = 0; i < this.operations.length; i++) {
            ...
            const selector = op.data.slice(0, 10); // 0x + 4 bytes
            const valid = isKeyValidForOperation(sessionKey, policy, op.target, selector, op.value ?? 0n);
            const validation = {
                index: i,
                valid,
            };
    ...
    encodeBatch(operations) {
        ...
        return encodedCalls.length > 0 ? encodedCalls[0] : "0x";
    }
}
// ============================================================
 ...
 */
export function encodeEnableSessionKeyBatch(sessionKey, policy, smartAccount) {
 ...
 */
export function encodeDisableSessionKey(sessionKey) {
    return encodeFunctionData({
        abi: [
            {
                type: "function",
