...
export class SessionKeyManager {
    ...
    generateKey(policy, label) {
        ...
        const sessionKey = {
            ...
            privateKey,
            expiresAt: policy?.expiresAt ?? now + 86400, // Default: 24 hours
            createdAt: now,
            policyId: policy?.id,
            label,
        };
     ...
     */
    importKey(privateKey, policy, label) {
        const account = privateKeyToAccount(privateKey);
        const now = Math.floor(Date.now() / 1000);
        const sessionKey = {
            publicKey: account.address,
            privateKey,
            expiresAt: policy?.expiresAt ?? now + 86400,
            createdAt: now,
            policyId: policy?.id,
            label,
        };
     ...
     */
    async signWithKey(publicKey, message) {
 ...
 */
export function encodeEnableSessionKey(sessionKey, policy) {
 ...
 */
export function isKeyValidForOperation(key, policy, target, method, amount) {
    // Check expiration
    if (key.expiresAt <= Math.floor(Date.now() / 1000)) {
        return false;
    }
