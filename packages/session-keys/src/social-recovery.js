...
export class SocialRecoveryManager {
     ...
     */
    initialize(guardians, threshold, recoveryDelay) {
        const effectiveThreshold = threshold ?? Math.max(3, Math.floor(guardians.length / 2) + 1);
        const effectiveDelay = recoveryDelay ?? 86400; // 24 hours
        if (effectiveThreshold > guardians.length) {
            throw new Error(`Threshold (${effectiveThreshold}) exceeds guardian count (${guardians.length})`);
        }
    ...
    removeGuardian(address) {
        ...
        const idx = this.recoveryConfig.guardians.findIndex((g) => g.address === address && g.isActive);
        if (idx === -1) {
     ...
     */
    addGuardianSignature(requestId, guardianAddress, signature) {
        ...
        const guardian = this.recoveryConfig?.guardians.find((g) => g.address === guardianAddress && g.isActive);
        if (!guardian) {
    ...
    canExecute(requestId) {
        ...
        if (request.executed || request.cancelled)
            return false;
        if (request.signatureCount < (this.recoveryConfig?.threshold ?? 0))
            return false;
        const now = Math.floor(Date.now() / 1000);
     ...
     */
    getPendingRequests() {
        return Array.from(this.pendingRequests.values()).filter((r) => !r.executed && !r.cancelled);
    }
    /**
     * Get all recovery requests for a specific account.
     */
    getRequestsForAccount(account) {
        return Array.from(this.pendingRequests.values()).filter((r) => r.account === account);
    }
    /**
...
// Helpers
// ============================================================
function generateRequestId(account, newOwner, timestamp) {
