...
export class WalletRegistry {
    ...
    getPopularWallets(limit = 20) {
        return this.getAllWallets()
            .filter(w => w.popular)
            .slice(0, limit);
    }
    getWalletsForChain(chainId) {
        return this.getAllWallets().filter(w => w.supportedChains.includes(chainId.split(':')[0]) ||
            w.supportedChains.includes(chainId));
    }
    getWalletsForPlatform(platform) {
        return this.getAllWallets().filter(w => w.platforms.includes(platform));
    }
    /* ── Search ──────────────────────────────────────────────── */
    search(query, type = 'wallet') {
        const q = query.toLowerCase();
        switch (type) {
            case 'wallet':
                return this.getAllWallets().filter(w => w.name.toLowerCase().includes(q) || w.id.includes(q));
            case 'dapp':
                return Array.from(this.dapps.values()).filter(d => d.name.toLowerCase().includes(q) || d.description?.toLowerCase().includes(q));
            case 'chain':
                return Array.from(this.chains.values()).filter(c => c.name.toLowerCase().includes(q) || c.caipNetworkId.includes(q));
        }
    }
    searchWallets(filter) {
        ...
        if (filter?.query) {
            results = this.search(filter.query, 'wallet');
        }
        return results;
    }
    /* ── Logo caching ────────────────────────────────────────── */
    getCachedLogo(walletId) {
        return this.logoCache.get(walletId);
    }
    cacheLogo(walletId, dataUri) {
