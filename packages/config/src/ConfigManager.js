...
export class ConfigManager {
    constructor(config) {
        ...
        this.features = { ...this.config.fallback };
    ...
    async fetchRemoteFlags() {
        try {
            ...
            if (!response.ok) {
                console.warn(`[ConfigManager] Remote fetch returned ${response.status} for project ${this.config.projectId}; using fallback.`);
                return;
            }
            const remote = await response.json();
            const previous = { ...this.features };
            this.features = { ...this.config.fallback, ...remote };
            this.notifyChanges(previous);
        }
        catch (err) {
            // Network error — keep using fallback.
            console.warn(`[ConfigManager] Failed to fetch remote flags:`, err instanceof Error ? err.message : err);
        }
    }
    ...
    startPolling() {
        ...
        const poll = async () => {
            ...
            this.abortController?.signal.addEventListener("abort", () => { }, { once: true });
            setTimeout(() => {
            ...
            if (prev !== curr) {
                for (const cb of callbacks) {
                    try {
                        ...
                        console.error(`[ConfigManager] Error in listener for "${flag}":`, err);
                    }
                }
            }
        }
