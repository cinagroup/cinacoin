/**
 * @cinaconnect/adapter-starknet
 *
 * Starknet chain adapter for the CinaConnect SDK.
 * Supports Argent X and Braavos wallets with native account abstraction.
 *
 * @packageDocumentation
 */
export { StarknetChainAdapter } from './StarknetAdapter.js';
export { ArgentXConnector } from './connectors/argent-x.js';
export { BraavosConnector } from './connectors/braavos.js';
export { STARKNET_CHAINS, STARKNET_WALLETS, type StarknetWalletInfo, type StarknetCall, type StarknetTransaction, type StarknetTransactionResult, type StarknetWalletConnector, type StarknetConnectParams, } from './types.js';
//# sourceMappingURL=index.d.ts.map