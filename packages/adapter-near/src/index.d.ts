/**
 * @cinaconnect/adapter-near
 *
 * NEAR chain adapter for the CinaConnect SDK.
 * Supports NEAR Wallet, Here Wallet, and Meteor Wallet.
 *
 * @packageDocumentation
 */
export { NearChainAdapter } from './NearAdapter.js';
export { NearWalletConnector } from './connectors/near-wallet.js';
export { HereWalletConnector } from './connectors/here-wallet.js';
export { NEAR_CHAINS, NEAR_WALLETS, type NearWalletInfo, type NearFunctionCall, type NearTransferAction, type NearTransaction, type NearTransactionResult, type NearWalletConnector as NearWalletConnectorInterface, type NearConnectParams, } from './types.js';
//# sourceMappingURL=index.d.ts.map