...
describe('initCommand', () => {
    ...
    it('should accept directory argument', async () => {
        ...
        const { initCommand } = await import('../src/commands/init.js');
        initCommand(mockCli);
        expect(mockCommand.argument).toHaveBeenCalledWith('[directory]', 'Project directory name', 'my-cinaconnect-app');
    });
    it('should accept template option', async () => {
        ...
        const { initCommand } = await import('../src/commands/init.js');
        initCommand(mockCli);
        expect(mockCommand.option).toHaveBeenCalledWith('--template <name>', 'Template to use (web | react | vue | next)', 'web');
    });
    it('should accept package-manager option', async () => {
        ...
        const { initCommand } = await import('../src/commands/init.js');
        initCommand(mockCli);
        expect(mockCommand.option).toHaveBeenCalledWith('--package-manager <pm>', 'Package manager (npm | yarn | pnpm)', 'pnpm');
    });
