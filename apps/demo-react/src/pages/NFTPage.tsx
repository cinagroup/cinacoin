...
const MOCK_NFTS = [
  ...
  {
    id: 2,
    name: 'Web3 Explorer #042',
    collection: 'Web3 Pioneers',
    image: 'WEB',
    traits: [
      { key: 'Level', value: 'Gold' },
      { key: 'Chain', value: 'Polygon' },
    ],
  },
  {
    id: 3,
    name: 'DeFi Master #108',
  ...
  {
    id: 4,
    name: 'Meta Avatar #555',
    collection: 'MetaVerse',
    image: 'AVT',
    traits: [
      { key: 'Style', value: 'Cyberpunk' },
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div>
        ...
        <p className="cc-body-md text-[var(--cc-body)] mb-6">
          Browse multi-chain NFT collections. Click to view details.
        </p>
          ...
          {MOCK_NFTS.length === 0 ? (
            <div className="col-span-full py-12 px-4 text-center cc-card">
              ...
              <p className="cc-body-sm text-[var(--cc-muted)]">
                Your NFT collection will appear here once you mint or receive NFTs.
              </p>
            </div>
          ) : (
            MOCK_NFTS.map((nft) => (
              <button
                ...
                className={`cc-card !p-4 text-left transition-all focus-ring ${
                  selectedNFT?.id === nft.id
                    ? 'border-[var(--cc-link)] ring-2 ring-[var(--cc-link)]/10'
                    : ''
                }`}
        ...
        {selectedNFT && (
          ...
          >
            <div onClick={(e) => e.stopPropagation()} className="cc-card-lg max-w-[400px] w-full">
              ...
              <div className="bg-[var(--cc-canvas-soft-2)] rounded-sm p-4 border border-[var(--cc-hairline)]">
                ...
                {selectedNFT.traits.map((trait) => (
                  <div key={trait.key} className="flex justify-between mb-2 last:mb-0">
                    ...
                    <span className="text-body-sm text-[var(--cc-ink)] font-semibold">
                      {trait.value}
                    </span>
                  </div>
                ))}
              ...
              <button onClick={() => setSelectedNFT(null)} className="cc-btn-secondary w-full mt-4">
                Close
              </button>
            </div>
          </div>
