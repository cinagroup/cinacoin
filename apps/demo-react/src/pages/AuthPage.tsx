...
const SOCIAL_PROVIDERS = [
  ...
  },
  {
    ...
    name: 'Apple',
    icon: (
      <svg className="w-5 h-5 text-[var(--cc-ink)]" fill="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    icon: (
      <svg
        className="w-5 h-5 text-[var(--cc-ink)]"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
...
Issued At: 2026-05-17T13:45:00.000Z`;
  ...
  const handleConnect = () => {
    setAddress('0x7a3F8C12dE4bF5678901234567890AbCdEfE82b');
    setStep(2);
  };
  const handleSign = () => {
    setSigning(true);
    ...
    <section className="max-w-xl mx-auto w-full py-12 px-4" aria-label="Authentication">
      <div className="mb-8">
        ...
        <p className="cc-body-md text-[var(--cc-body)]">
          Sign in with your wallet or social account. No passwords needed.
        </p>
      </div>
          ...
          >
            {['Connect', 'Sign', 'Verify', 'Profile'].map((label, i) => (
              <div key={label} className="flex items-center gap-2">
                ...
                {i < 3 && (
                  <div
                    className={`w-8 h-px ${step > i + 1 ? 'bg-[var(--cc-success)]/30' : 'bg-[var(--cc-hairline)]'}`}
                  />
                )}
              </div>
            ))}
                ...
                {WALLET_PROVIDERS.map((w) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <span className="font-semibold text-body-sm text-[var(--cc-ink)]">
                        {w.name}
                      </span>
                      <p className="text-caption text-[var(--cc-muted)]">Connect with {w.name}</p>
                    </div>
                    <span
                      className="text-[var(--cc-muted)] group-hover:translate-x-0.5 transition-transform"
                      aria-hidden="true"
                    >
                      →
                    </span>
          ...
          {step === 3 && (
            <div className="cc-card space-y-4" role="tabpanel" aria-labelledby="tab-wallet">
              ...
              <div className="bg-[var(--cc-canvas-soft-2)] rounded-sm p-4 border border-[var(--cc-hairline)] text-body-sm space-y-2">
                ...
                <div className="flex justify-between">
                  <span className="text-[var(--cc-muted)]">Signature</span>
                  <span className="font-[var(--font-mono)] text-caption text-[var(--cc-body)]">
                    0x1a2b3c4d...
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--cc-muted)]">Nonce</span>
                  <span className="font-[var(--font-mono)] text-caption text-[var(--cc-body)]">
                    8b3f9a2c
                  </span>
                </div>
      ...
      {authMethod === 'social' && (
        <div
          className="cc-card space-y-6 animate-fade-in"
          role="tabpanel"
        ...
        >
          <div className="text-center">
            ...
            <p className="cc-body-sm text-[var(--cc-muted)] mt-1">
              Connect with your favorite account. We'll create a wallet for you.
            </p>
          </div>
          ...
          {socialLoading && (
            <div className="flex flex-col items-center gap-3 py-6" role="status" aria-live="polite">
              ...
              <p className="text-body-sm text-[var(--cc-muted)]">
                Connecting to {SOCIAL_PROVIDERS.find((s) => s.id === selectedSocial)?.name}...
              </p>
            </div>
          )}
          ...
          {selectedSocial && !socialLoading && step === 4 && (
            <div className="bg-[var(--cc-canvas-soft-2)] rounded-sm p-4 border border-[var(--cc-success)]/20">
              <div className="text-center mb-3">
                ...
                <h3 className="text-body-lg font-semibold text-[var(--cc-success)]">
                  Authenticated.
                </h3>
              </div>
              <div className="space-y-2 text-body-sm">
                <div className="flex justify-between">
                  <span className="text-[var(--cc-muted)]">Provider</span>
                  <span className="font-semibold">
                    {SOCIAL_PROVIDERS.find((s) => s.id === selectedSocial)?.name}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--cc-muted)]">Wallet</span>
                  <span className="font-[var(--font-mono)] text-caption">{address}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--cc-muted)]">Verified</span>
                  <span className="text-[var(--cc-success)] font-semibold">
                    <Check className="w-4 h-4 inline" /> True
                  </span>
                </div>
              </div>
              <button
      ...
      {step === 4 && verified && (
        <div className="cc-card mt-6 border-t-2 border-t-[var(--cc-success)]">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="font-[var(--font-mono)] text-body-sm text-[var(--cc-ink)] font-semibold">
                {address}
              </p>
              <p className="text-caption text-[var(--cc-muted)] mt-1">
                {authMethod === 'social'
                  ? `Signed in via ${SOCIAL_PROVIDERS.find((s) => s.id === selectedSocial)?.name}`
          ...
          >
            <thead>
              <tr className="border-b border-[var(--cc-hairline)] text-[var(--cc-muted)] text-caption bg-[var(--cc-canvas-soft-2)]">
                <th scope="col" className="text-left p-3 font-semibold cc-caption-mono">
                  Feature
                </th>
            ...
            <tbody>
              {[
                ...
                <tr
                  key={f as string}
                  className="border-b border-[var(--cc-hairline)]/50 hover:bg-[var(--cc-canvas-soft-2)]/30 transition-colors"
                >
                  <td className="p-3 font-medium text-[var(--cc-body)]">{f as string}</td>
...
// Passkey login
const wallet = await cc.passkeyLogin();`}
        </pre>
      </div>
    </section>
  );
