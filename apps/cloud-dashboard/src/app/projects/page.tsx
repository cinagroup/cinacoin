      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-[var(--cc-canvas-soft)] border border-[var(--cc-hairline)] rounded-sm text-body-sm w-80 focus:outline-none focus:border-link focus:ring-2 focus:ring-link/10 transition-colors"
              />
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 p-6 overflow-auto">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <p className="font-mono text-xs text-[var(--cc-muted)] mb-2">WORKSPACE</p>
              <h1 className="text-heading-2 text-[var(--cc-ink)]">Projects.</h1>
          ...
          <div className="grid gap-4">
            {mockProjects.length === 0 ? (
              <div className="bg-[var(--cc-canvas)] border border-[var(--cc-hairline)] rounded-md p-12 text-center">
                <FolderOpen className="w-8 h-8 text-[var(--cc-muted)] mb-4 mx-auto" />
                <h3 className="text-heading-3 text-[var(--cc-ink)] mb-1">No projects yet.</h3>
                <p className="text-body-sm text-body max-w-sm">Create your first project to get started with cloud services.</p>
              </div>
            ) : mockProjects.map((project) => (
              <div key={project.id} className="bg-[var(--cc-canvas)] border border-[var(--cc-hairline)] rounded-md p-6 hover:shadow-level-2 transition-shadow">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-heading-3 text-[var(--cc-ink)]">{project.name}</h3>
                      <span className="badge badge-success">{project.status}</span>
                    </div>
                    <div className="grid grid-cols-3 gap-4 mt-4 text-body-sm">
                      <div>
                        <span className="text-[var(--cc-muted)]">Resources:</span>
                        <span className="ml-2 text-[var(--cc-ink)] font-medium">{project.resources}</span>
                      </div>
                      <div>
                        <span className="text-[var(--cc-muted)]">Region:</span>
                        <span className="ml-2 text-[var(--cc-ink)] font-medium">{project.region}</span>
                      </div>
                      <div>
                        <span className="text-[var(--cc-muted)]">Created:</span>
                        <span className="ml-2 text-[var(--cc-ink)] font-medium">{project.created}</span>
                      </div>
                    </div>
                  </div>
