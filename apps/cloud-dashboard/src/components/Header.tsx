  ...
  return (
    <header className="cc-navbar relative">
        ...
        <nav className="hidden sm:flex items-center gap-4 lg:gap-6" aria-label="Primary navigation">
          {navItems.map((item) => {
            ...
            <Link
              key={item.href}
              href={item.href}
              className={`text-body-sm font-medium transition-colors px-2 py-1 rounded-[var(--app-radius)] ${
                isActive
      ...
      {mobileOpen && (
        <>
          <div
            className="fixed inset-0 bg-[var(--cc-ink)]/60 backdrop-blur-sm z-40 sm:hidden"
