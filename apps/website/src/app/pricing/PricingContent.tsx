                    ...
                    <ul className="mt-8 space-y-3">
                      {tier.features.map((feature) => (
                        <li key={feature} className="flex items-start gap-3 cc-body-sm text-[var(--cc-body)]">
                          <span className="mt-1 text-[var(--cc-success)]" aria-hidden="true"><svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></span>
                          {feature}
                        </li>
                      ))}
                    </ul>
            ...
            <FadeIn key={i} delay={i * 100}>
              <details className="group border-b border-[var(--cc-hairline)] py-6 last:border-b-0">
                <summary className="flex cursor-pointer items-center justify-between cc-body-md-strong transition-colors hover:text-[var(--cc-link)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--cc-link)] focus-visible:ring-offset-2 rounded">
                  {t(faq.qKey)}
                  <span
                    className="ml-4 text-[var(--cc-muted)] transition-transform group-open:rotate-45 text-display-sm"
                    aria-hidden="true"
                  >
                    +
                  </span>
