...
export function SpinnerRing({ size = 'md', color = 'text-[var(--cc-link)]' }: { size?: 'sm' | 'md' | 'lg'; color?: string }) {
  const sizeClasses = {
    sm: 'w-4 h-4',
    md: 'w-6 h-6',
    lg: 'w-8 h-8',
...
export function SpinnerPulse({ size = 'md', color = 'text-[var(--cc-link)]' }: { size?: 'sm' | 'md' | 'lg'; color?: string }) {
  const sizeClasses = {
    sm: 'w-8 h-8',
    md: 'w-12 h-12',
    lg: 'w-16 h-16',
...
export function LoadingOverlay({ message = 'Loading...' }: { message?: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-4" role="status" aria-label={message}>
      ...
      <p className="text-body-sm text-[var(--cc-muted)] animate-pulse">{message}</p>
    </div>
  );
}
