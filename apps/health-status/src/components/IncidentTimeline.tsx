          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-0.5 text-sm text-[var(--cc-body)]">{incident.date}</p>
              </div>
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${colors.text} ${colors.bg}`}>
                {incident.status}
              </span>
            </div>
            <div className="mt-4 space-y-3 border-l-2 border-[var(--cc-hairline)] pl-4">
              {incident.updates.map((update, idx) => (
                <div key={idx} className="relative">
                  ...
                  <p className="text-sm text-[var(--cc-body)]">{update.message}</p>
                  <p className="mt-0.5 text-xs text-[var(--cc-muted)]">{update.timestamp}</p>
                </div>
              ))}
            </div>
          </div>
