/**
 * Cloudflare Pages Function - Transfer Frame POST handler.
 */
export async function onRequestPost(context: EventContext) {
  ...
  try {
    ...
    switch (buttonIndex) {
      case 1: {
        // Send
        const isValid = /^0x[a-fA-F0-9]{40}$/.test(inputText);
        if (!isValid && inputText) {
          return frameResponse({
            image: `${appUrl}/og-transfer.png`,
            title: '⚠️ Invalid address format',
            input: 'Enter valid 0x address...',
            buttons: [
        ...
        if (inputText) {
          return frameResponse({
            image: `${appUrl}/og-transfer.png`,
            title: `Send to ${inputText.slice(0, 6)}...${inputText.slice(-4)}?`,
            buttons: [
        ...
        return frameResponse({
          image: `${appUrl}/og-transfer.png`,
          title: 'Enter a recipient address',
