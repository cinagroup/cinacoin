        ...
        <div className="space-y-2">
          {dailyRequests.map((day) => (
            <div key={day.date} className="flex items-center gap-3">
              ...
              <span className="text-caption text-ink-body w-16 text-right shrink-0">
                {(day.requests / 1000).toFixed(1)}k
              </span>
            </div>
          ))}
        </div>
        <div className="flex gap-4 mt-3 text-caption text-ink-mute">
        ...
        <div className="space-y-3">
          {topChains.map((chain) => (
            <div key={chain.name} className="flex items-center gap-4">
              ...
              <span className="text-body-sm text-ink-body w-28 text-right shrink-0">
