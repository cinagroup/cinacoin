...
export default function BillingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink">Billing & Quotas</h1>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-1 mb-2">
              <span className="text-3xl font-bold text-ink">{currentPlan.price}</span>
              <span className="text-ink-mute">{currentPlan.period}</span>
            </div>
            <p className="text-sm text-ink-body mb-3">{currentPlan.description}</p>
            <ul className="space-y-1">
              {currentPlan.features.map((feature) => (
                <li key={feature} className="text-sm text-ink-body flex items-center gap-2">
          ...
          <div className="text-right">
            <div className="text-sm text-ink-mute mb-2">Next billing date</div>
            <div className="text-sm font-medium text-ink">July 10, 2026</div>
            <button
      ...
      {/* Usage vs Quota */}
      <div>
        <h2 className="text-lg font-semibold text-ink mb-4">Usage vs Quota</h2>
          ...
          <div className="modal-content max-w-4xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {availablePlans.map((plan) => (
                ...
                >
                  {plan.current && (
                    <div className="text-xs font-medium text-ink mb-2">Current Plan</div>
                  )}
                  <h3 className="text-lg font-semibold text-ink mb-1">{plan.name}</h3>
                  <div className="flex items-baseline gap-1 mb-2">
                    <span className="text-2xl font-bold text-ink">{plan.price}</span>
                    <span className="text-sm text-ink-mute">{plan.period}</span>
                  </div>
                  <p className="text-sm text-ink-body mb-4">{plan.description}</p>
                  <ul className="space-y-2 mb-4">
                    {plan.features.map((feature) => (
                      <li key={feature} className="text-sm text-ink-body flex items-start gap-2">
