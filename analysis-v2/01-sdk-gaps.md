...
**Date:** 2026-05-17  
**Comparison:** CinaAuth/Cinacoin (34 packages) vs Cinacoin AppKit (20 packages + 8 adapters)  
**Cinacoin Version:** 1.7.1 (from `package.json` on GitHub main)  
**CinaAuth Version:** 0.1.0 (from core-sdk/index.ts VERSION constant)

---

## 1. Cinacoin AppKit Full Package Inventory

### Core Packages (12)

| Package | NPM Name | Purpose |
|---------|----------|---------|
| `appkit` | `@cinacoin/appkit` | Core SDK — chain-agnostic orchestration, config, createAppKit() |
| `appkit-utils` | `@cinacoin/appkit-utils` | Shared utilities (Constants, Types, AssetUtil, CoreHelperUtil) |
| `common` | `@cinacoin/common` | Shared types, constants, error codes across all packages |
| `controllers` | `@cinacoin/appkit-controllers` | State management (AccountController, ConnectionController, ThemeController, etc.) |
| `ui` | `@cinacoin/ui` | UI primitives — LitElement Web Components (buttons, icons, modals) |
| `scaffold-ui` | `@cinacoin/appkit-scaffold-ui` | Connect modal scaffold — views, partials, modal orchestration |
| `siwe` | `@cinacoin/appkit-siwe` | Sign-In with Ethereum (CAIP-222) |
| `siwx` | `@cinacoin/appkit-siwx` | Sign-In with X — chain-agnostic authentication |
| `pay` | `@cinacoin/appkit-pay` | Payment processing |
| `universal-connector` | `@cinacoin/appkit-universal-connector` | Universal wallet connection |
| `wallet` | `@cinacoin/appkit-wallet` | Wallet management |
| `wallet-button` | `@cinacoin/appkit-wallet-button` | Wallet button component |

### Tooling Packages (4)

| Package | NPM Name | Purpose |
|---------|----------|---------|
| `cli` | `@cinacoin/appkit-cli` | CLI tool for setup/scaffolding |
| `codemod` | `@cinacoin/appkit-codemod` | Migration codemod (walletconnect→cinacoin, wagmi/viem updates) |
| `cdn` | `@cinacoin/appkit-cdn` | CDN bundle for script-tag usage |
| `testing` | `@cinacoin/appkit-testing` | Testing utilities/mocks |

### Infrastructure Packages (3)

| Package | NPM Name | Purpose |
|---------|----------|---------|
| `core-legacy` | `@cinacoin/appkit-core-legacy` | Legacy Cinacoin v1 compatibility |
| `experimental` | `@cinacoin/appkit-experimental` | Experimental features |
| `polyfills` | `@cinacoin/appkit-polyfills` | Node/browser polyfills |

### Chain Adapters (8)

| Package | NPM Name | Purpose |
|---------|----------|---------|
| `adapters/bitcoin` | `@cinacoin/appkit-adapter-bitcoin` | Bitcoin (Bip122) chain adapter |
| `adapters/ethers` | `@cinacoin/appkit-adapter-ethers` | Ethers.js v6 adapter |
| `adapters/ethers5` | `@cinacoin/appkit-adapter-ethers5` | Ethers.js v5 adapter |
| `adapters/polkadot` | `@cinacoin/appkit-adapter-polkadot` | Polkadot/Substrate chain adapter |
| `adapters/solana` | `@cinacoin/appkit-adapter-solana` | Solana chain adapter |
| `adapters/ton` | `@cinacoin/appkit-adapter-ton` | TON chain adapter |
| `adapters/tron` | `@cinacoin/appkit-adapter-tron` | TRON chain adapter |
| `adapters/wagmi` | `@cinacoin/appkit-adapter-wagmi` | Wagmi/viem adapter (primary EVM path) |
...
| `react-native` | `@cinacoin/react-native` | 2,314 | 615 | 9+3 | React Native — native components, QRScanner, Cinacoin provider |
...
| Dimension | Cinacoin `appkit` | CinaAuth `core-sdk` | Verdict |
|-----------|---------------|---------------------|---------|
...
| Dimension | Cinacoin `ui` + `scaffold-ui` | CinaAuth `core-ui` | Verdict |
|-----------|---------------------------|-------------------|---------|
...
| LOC (src) | ~15K+ combined | 3,427 | Cinacoin larger |
| Tests | Part of monorepo | 1,867 | Cina well-tested |

#### `adapters/wagmi` → `core-sdk/adapters/wagmi.ts` ✅

| Dimension | Cinacoin | CinaAuth | Verdict |
|-----------|-------|----------|---------|
| LOC | ~4,000+ (single client.ts is 35KB) | 428 | Cinacoin much larger |
...
**All 8 Cinacoin chain adapters have equivalents in CinaAuth core-sdk.**

| Adapter | Cinacoin LOC (est.) | CinaAuth LOC | Features |
|---------|------------------|--------------|----------|
...
**CinaAuth ADDS 3 more adapters not in Cinacoin:**
...
| Dimension | Cinacoin `siwe` | CinaAuth `siwe` | Verdict |
|-----------|-------------|-----------------|---------|
...
| Dimension | Cinacoin `siwx` | CinaAuth `siwx` | Verdict |
|-----------|-------------|-----------------|---------|
...
| Dimension | Cinacoin `cli` | CinaAuth `cli` | Verdict |
|-----------|-------------|-----------------|---------|
| LOC (src) | ~1,000+ | 628 | Cinacoin larger |
...
| Dimension | Cinacoin `wallet` | CinaAuth `walletconnect-v2` | Verdict |
|-----------|---------------|----------------------------|---------|
...
Cinacoin's appkit-utils (constants, types, helpers) are absorbed into CinaAuth's core-sdk.

#### `common` → `core-sdk` (absorbed) ✅

Cinacoin's common package (shared types, errors) is absorbed into CinaAuth's core-sdk types.

#### `controllers` → `core-sdk/store.ts` ✅

Cinacoin's controllers (AccountController, ConnectionController, etc.) map to CinaAuth's store.ts with CinacoinState management.
...
| Dimension | Cinacoin `pay` | CinaAuth | Verdict |
|-----------|-------------|----------|---------|
...
| Dimension | Cinacoin | CinaAuth | Verdict |
|-----------|-------|----------|---------|
| Purpose | Migration scripts (walletconnect→cinacoin, wagmi/viem upgrades) | None | **Missing** |
...
| Dimension | Cinacoin | CinaAuth | Verdict |
|-----------|-------|----------|---------|
| Purpose | Cinacoin v1 backwards compatibility | None | **Missing (intentionally)** |
...
These are **CinaAuth differentiators** — features Cinacoin doesn't offer:

| Package | LOC (src) | Purpose | Competitive Advantage |
|---------|-----------|---------|----------------------|
| **`session-keys`** | 1,793 | ERC-4337 session keys with policies, social recovery | 🟢 **Major** — Cinacoin lacks this |
| **`social-login`** | 1,131 | OAuth2 (Google, Apple, Twitter/X, email) with HD wallet derivation | 🟢 **Major** — Cinacoin has email/social but less comprehensive |
| **`passkey-auth`** | 645 | WebAuthn passkey authentication | 🟢 **Major** — Cinacoin has no passkey support |
| **`swap-sdk`** | 1,213 | Multi-DEX swap aggregator (Uniswap, 1inch, 0x) with slippage protection | 🟢 **Major** — Cinacoin swaps are basic |
| **`onramp-sdk`** | 1,064 | Multi-provider on-ramp aggregator (MoonPay, Ramp, Transak) | 🟢 **Major** — Cinacoin pay is simpler |
| **`cross-chain-sync`** | 735 | Cross-chain identity/state sync across 6 chains | 🟢 **Major** — unique feature |
| **`batch-transaction`** | 481 | Atomic batch operations with typed operations | 🟡 Medium — Cinacoin lacks |
| **`gas-estimator`** | 409 | Gas estimation for EVM + Solana | 🟡 Medium — Cinacoin lacks |
| **`token-list`** | 569 | Token discovery from multiple sources with caching | 🟡 Medium — Cinacoin lacks |
| **`wallet-recommender`** | 373 | Wallet recommendation engine | 🟡 Medium — Cinacoin lacks |
| **`analytics`** | 885 | GDPR-compliant analytics with consent management | 🟡 Medium — Cinacoin lacks |
| **`design-tokens`** | ~500 | Design token system (3 themes, CSS variables) | 🟡 Medium — Cinacoin UI is less themable |
| **`ens-resolver`** | ~100 | ENS contract resolution | 🔵 Minor — nice to have |
| **`aa-sdk`** | 486 | Account Abstraction smart account management | 🟡 Medium — Cinacoin has AA but less standalone |
...
| **`relay-server`** | Rust | ~2,183 | Cinacoin relay server (self-hosted) | 🟢 **Major** — no Cinacoin infra dependency |
...
**Strategic advantage:** CinaAuth offers fully self-hosted infrastructure. Cinacoin requires connection to their proprietary servers.

### 3.6 📱 CINA NATIVE MOBILE PACKAGES (PARITY WITH REOWN)

| Package | Language | LOC | Tests | Cinacoin Equivalent |
|---------|----------|-----|-------|-----------------|
| `android-kotlin` | Kotlin | ~3,081 | 7 | Cinacoin Android SDK ✅ |
| `ios-swift` | Swift | ~5,537 | 0 (no test files found) | Cinacoin iOS SDK ✅ |
| `flutter-dart` | Dart | ~4,397 | 9 | Cinacoin Flutter SDK ✅ |
| `unity-csharp` | C# | ~3,084 | 0 (no test files found) | Cinacoin Unity SDK ✅ |

**Notes:**
- CinaAuth has iOS and Unity packages with **zero test coverage** — needs attention.
- Cinacoin also supports Svelte and Nuxt — CinaAuth does not.
...
| Chain | Cinacoin Adapter | CinaAuth Adapter | Cina LOC | Verdict |
|-------|--------------|------------------|----------|---------|
...
| Component | Cinacoin | CinaAuth | Notes |
|-----------|-------|----------|-------|
...
| Package | Cinacoin Tests | CinaAuth Tests (LOC) | Ratio (test/src) | Verdict |
|---------|------------|---------------------|-------------------|---------|
...
| session-keys | N/A (no Cinacoin equiv) | 1,039 | 58% | Good |
...
| Cinacoin Package | CinaAuth Equivalent | Status | Notes |
|--------------|--------------------|--------|-------|
...
| — | `android-kotlin` | 🆕 | Native parity with Cinacoin |
| — | `ios-swift` | 🆕 | Native parity with Cinacoin |
| — | `flutter-dart` | 🆕 | Native parity with Cinacoin |
| — | `unity-csharp` | 🆕 | Native parity with Cinacoin |
| — | `react-native` | 🆕 | Cina exclusive |

### Quantitative Summary

| Metric | Cinacoin AppKit | CinaAuth/Cinacoin | Delta |
|--------|-------------|-------------------|-------|
...
| UI framework bindings | React, Vue, Svelte, Nuxt, JS | React, Vue, React Native | Cinacoin +2 (Svelte, Nuxt) |
| Total JS/TS LOC (src) | ~80,000+ (est.) | ~25,000+ | Cinacoin larger |
| Total test LOC | Monorepo-wide | ~18,000+ | Cina well-tested |
| Self-hosted infra | ❌ (requires Cinacoin servers) | ✅ (4 Rust servers) | **Cina major advantage** |

### Priority Recommendations

#### 🔴 Critical Gaps (Fill First)
1. **`testing`** — Build test utilities package for library consumers. Cinacoin has this; CinaAuth doesn't provide a standalone testing package.
...
#### 🟡 Important Gaps
4. **`codemod`** — Build migration tool for Cinacoin → Cinacoin. Critical for adoption from existing WC users.
5. **`polyfills`** — Add Node/browser polyfills for bundler compatibility.
6. **`cdn`** — Build CDN bundle for zero-build script-tag usage.

#### 🟢 Nice-to-Have
7. **`svelte` bindings** — Cinacoin supports Svelte; CinaAuth doesn't.
8. **`nuxt` bindings** — Cinacoin supports Nuxt; CinaAuth doesn't.
...
- **13 exclusive packages** with no Cinacoin equivalent (session-keys, social-login, passkey-auth, swap-sdk, onramp-sdk, cross-chain-sync, batch-transaction, gas-estimator, token-list, wallet-recommender, analytics, design-tokens, aa-sdk)
- **6 self-hosted Rust servers** — complete infrastructure independence
- **3 extra chain adapters** (ethers v6, viem, generic EVM)
- **React Native support** (Cinacoin has it but Cina's implementation is more comprehensive with native QRScanner)
