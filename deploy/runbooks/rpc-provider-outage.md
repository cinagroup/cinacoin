...
```bash
kubectl logs -l app=rpc-proxy -n onchainux --tail=200 | grep -i error
```
...
```bash
kubectl edit configmap rpc-proxy-config -n onchainux
...
kubectl scale statefulset erigon-ethereum -n onchainux --replicas=3
```
...
kubectl exec -it deploy/rpc-proxy -n onchainux -- curl -X POST http://localhost:8545/admin/update-providers \
  -H "Content-Type: application/json" \
