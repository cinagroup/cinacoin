...
```bash
kubectl logs statefulset/erigon-ethereum -n onchainux --tail=500 | grep -iE "sync|import|error"
```

### Step 3: Check Disk I/O

```bash
kubectl exec -it erigon-ethereum-0 -n onchainux -- iostat -x 5 3
```
...
```bash
kubectl top pods -n onchainux -l app=erigon-ethereum
...
kubectl edit statefulset erigon-ethereum -n onchainux
# Add: --p2p.maxpeers=200
kubectl rollout restart statefulset erigon-ethereum -n onchainux
```
...
# 1. Stop the node
kubectl scale statefulset erigon-ethereum -n onchainux --replicas=0

# 2. Download latest snapshot to PVC
# (Use official Erigon snapshots)

# 3. Restart
kubectl scale statefulset erigon-ethereum -n onchainux --replicas=2
```
