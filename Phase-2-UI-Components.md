...
> **时间**: M2-M4  
> **产出物**: `@onchainux/ui` 组件库 + 多框架适配器 + 示例应用
...
```tsx
// React 组件封装
import { OnChainUXProvider, ConnectButton, ConnectModal } from '@onchainux/react';
...
// Hook
function useOnChainUX() {
  const { connectors, connect, disconnect, account, chainId, status } = useContext(OnChainUXContext);
...
```vue
<script setup>
import { OnChainUXProvider } from '@onchainux/vue'
...
```tsx
// 原生 RN 组件 (非 Web 包装)
import { OnChainUXProvider, ConnectButton, ConnectModal } from '@onchainux/react-native';
