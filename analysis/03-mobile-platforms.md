...
> **CinaAuth/Cinacoin vs Cinacoin (ex-Cinacoin)**
...
Cinacoin has invested years into building **six distinct native/mobile SDKs** covering the full spectrum of mobile and game development platforms. CinaAuth/Cinacoin currently has **one mobile adapter** — a React Native wrapper built with stub/mock connection logic.

The gap is **structural, not cosmetic**. Cinacoin's RN package (v0.1.0, 5 source files) provides UI component scaffolding only — the actual wallet connection layer (`connect()`, `disconnect()`, `switchChain()`) is mocked with `setTimeout`. There is no Cinacoin protocol integration, no injected wallet detection, no deep linking, and no push notification support.
...
| Platform | Cinacoin SDK | CinaAuth/Cinacoin | Gap |
|---|---|---|---|
| **iOS (Swift)** | cinacoin-swift (108★, 105 forks) | ❌ None | **Critical** |
| **Android (Kotlin)** | cinacoin-kotlin (56★, 29 forks) | ❌ None | **Critical** |
| **React Native** | appkit-react-native (123★, 40 forks) | ✅ @cinacoin/react-native v0.1.0 | **Major** (stub logic) |
| **Flutter** | cinacoin_flutter (59★, 65 forks) | ❌ None | **Critical** |
| **Unity / .NET** | cinacoin-dotnet (36★, 13 forks) | ❌ None | **Major** |
| **Rust (native lib)** | cinacoin-rust (41★, 17 forks) | ❌ None | **Major** |
| **Web (JS)** | appkit (appkit.js) | ✅ @cinacoin/react, @cinacoin/vue | Moderate |
| **Web (WC)** | appkit-web | ⚠️ @cinacoin/core-ui (planned) | Moderate |

**Score**: Cinacoin covers 8 platform targets. Cinacoin covers 3 (RN, React, Vue), with only web targets being functional.
...
| Feature | Cinacoin appkit-react-native | Cinacoin react-native | Gap |
|---|---|---|---|
...
| Cinacoin v2 | Full protocol (pair, connect, approve) | ❌ Mocked with setTimeout | **Critical** |
...
| Chain switching | Full Cinacoin `wallet_switchEthereumChain` | ⚠️ Mock setTimeout | Major |
...
| Metric | Cinacoin appkit-react-native | Cinacoin react-native |
|---|---|---|
...
| Documentation | Cinacoin docs site | ❌ None beyond JSDoc |
...
No dependencies on Cinacoin libraries, no crypto, no networking, no native modules.

---

## 3. Native iOS/Android SDK Gap

### 3.1 What Cinacoin Offers (cinacoin-swift + cinacoin-kotlin)

- Full Cinacoin v2 protocol implementation in native Swift/Kotlin
...
- Support for Cinacoin v2 Relay network (WebSocket connections)
- Signing requests (personal_sign, eth_signTypedData, eth_sendTransaction)
- Native QR code generation for Cinacoin URI display
...
1. **Cinacoin v2 Protocol Client** (~2-3 months per platform)
...
### 3.3 Strategic Question: Build Native vs. Use Cinacoin's SDK?

Since Cinacoin's core connects *through* Cinacoin v2 (the protocol is open), the choice is:

| Approach | Pros | Cons |
|---|---|---|
| **Build from scratch** | Full IP ownership, no Cinacoin dependency | 12-18 months for 4 platforms |
| **Fork Cinacoin SDKs** | Faster start (weeks, not months) | License compliance, upstream drift |
| **Use Cinacoin SDK under the hood** | Fastest path (days), reliable | Brand dependency, API lock-in |

**Recommendation**: For Phase 2-3, build a thin abstraction layer that can wrap either a self-built implementation or a Cinacoin SDK underneath. This preserves the white-label goal while providing a realistic timeline.

---

## 4. Flutter / Unity / Rust Gap

### 4.1 Flutter (cinacoin_flutter: 59★, 65 forks)

Cinacoin's Flutter SDK provides:
- Full Cinacoin v2 via platform channels to native Kotlin/Swift
...
### 4.2 Unity / .NET (cinacoin-dotnet: 36★, 13 forks)

Cinacoin's .NET SDK targets:
- Unity game engine (mobile games, web3 games)
- .NET MAUI for cross-platform mobile apps
- Full Cinacoin v2 protocol in C#
...
### 4.3 Rust (cinacoin-rust: 41★, 17 forks)

Cinacoin's Rust SDK provides:
- Native Cinacoin v2 implementation in Rust
...
| Chain | Cinacoin (all platforms) | Cinacoin RN |
|---|---|---|
...
Cinacoin's push notification infrastructure is a core advantage — it allows wallets to receive connection requests and signing requests even when the app is backgrounded, via the Cinacoin Relay service.
...
| Feature | Cinacoin | Cinacoin RN |
|---|---|---|
| Cinacoin v2 pairing | ✅ Full protocol | ❌ Mock |
| Cinacoin v2 session management | ✅ Full lifecycle | ❌ Mock |
...
| Migration guides | Cinacoin→Cinacoin migration | N/A |
...
| **P1** | RN: Multi-chain support | 2-3 weeks | High | Implement `wallet_switchEthereumChain` via Cinacoin. Add Solana chain adapter. |
...
| **P2** | Native Swift iOS SDK | 3-4 months | High | Build or fork cinacoin-swift. Start with ConnectButton + ConnectModal + WCv2 protocol. |
| **P2** | Native Kotlin Android SDK | 3-4 months | High | Build or fork cinacoin-kotlin. Same scope as iOS. |
...
2. Rewrite `CinacoinProvider.connect()` to establish real Cinacoin v2 sessions
...
1. **Choose build vs. fork**: Evaluate Cinacoin SDK licenses (Apache 2.0) for forking feasibility
...
```
cinacoin-rust-core          # Rust Cinacoin v2 implementation
...
This mirrors Cinacoin's approach (cinacoin-rust as the foundation) and dramatically reduces per-platform development effort.
