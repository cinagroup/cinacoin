# Gap Analysis: CinaAuth/OnChainUX vs Reown AppKit

> **Date**: 2026-05-16
> **Scope**: SDK Architecture & Core Functionality Comparison
> **CinaAuth Project**: `/home/cina/.openclaw/workspace/onchainux/` (all design docs + code)
...
| Aspect | Reown AppKit | CinaAuth OnChainUX | Gap |
|--------|-------------|-------------------|-----|
...
The store creates a fresh instance on every `createOnChainUXStore()` call with no singleton pattern. This could lead to multiple disconnected store instances in complex apps.
...
| Feature | Reown AppKit | OnChainUX | Status |
|---------|-------------|-----------|--------|
...
OnChainUX's `SessionManager` implements a basic state machine (`disconnected → connecting → connected → disconnected`). However:
...
| Chain/Ecosystem | Reown AppKit | OnChainUX | Gap |
|-----------------|-------------|-----------|-----|
...
**Critical Gap**: OnChainUX has a single `EvmAdapter` class that makes raw EIP-1193 calls. It lacks:
...
| Aspect | Reown AppKit | OnChainUX | Gap |
|--------|-------------|-----------|-----|
...
| Component | Reown AppKit | OnChainUX | Status |
|-----------|-------------|-----------|--------|
...
**Analysis**: OnChainUX has exactly **one test file** — `Paymaster.t.sol` with Foundry tests for the paymaster contracts. This is actually a well-written test suite. However, there are **zero** TypeScript tests across all SDK packages. The CI pipeline runs `npm test` but there are no test files to run.

---

## 7. Developer Tooling

| Tool | Reown AppKit | OnChainUX | Gap |
|------|-------------|-----------|-----|
...
**Analysis**: Reown AppKit invests heavily in developer experience. The CLI enables quick setup, the codemod enables migration from older versions, and the laboratory app serves as an interactive testing harness. OnChainUX has none of this.
...
OnChainUX actually **exceeds** Reown AppKit in infrastructure design scope:

| Aspect | Reown AppKit | OnChainUX | Winner |
|--------|-------------|-----------|--------|
| Self-hosted relay | Reown's proprietary relay | Full design + Rust implementation | 🟢 OnChainUX |
| RPC proxy | None (uses WC/Alchemy) | Full design + Go implementation | 🟢 OnChainUX |
| K8s deployment | Not applicable | Helm charts, multi-region | 🟢 OnChainUX |
| Monitoring | Not part of SDK | Prometheus, Grafana, runbooks | 🟢 OnChainUX |
| Smart accounts | Not part of AppKit | Bundler + Paymaster + design | 🟢 OnChainUX |
| Swap aggregation | Not part of AppKit | Swap SDK (1inch, Uniswap, 0x) | 🟢 OnChainUX |
| On-ramp aggregation | Not part of AppKit | OnRamp SDK (MoonPay, Ramp, Transak) | 🟢 OnChainUX |

**Note**: These are not direct comparisons — Reown AppKit is a frontend SDK, while OnChainUX aspires to be a full-stack infrastructure platform. The infrastructure advantage is only theoretical until implemented.
...
| P0-2 | **Implement SIWE/SIWX authentication** | EIP-4361 is table stakes for dApp auth. Reown has it, competitors have it. OnChainUX has zero. | 2-3 weeks |
...
| P2-2 | **Add CLI tool** (`@onchainux/cli`) | Developer onboarding experience. | 2-3 weeks |
...
| Dimension | Reown AppKit | OnChainUX | Gap Severity |
|-----------|-------------|-----------|-------------|
...
OnChainUX has **exceptional architectural vision** — the design docs are thorough, well-structured, and cover areas that Reown AppKit doesn't even touch (self-hosted relay, RPC proxy, smart accounts, swap aggregation, on-ramp aggregation). The infrastructure design is arguably superior to what a typical dApp team would build.

However, the **implementation reality** is that OnChainUX is at approximately **5-10% of Reown AppKit's functional capability** in core SDK areas. The most critical blockers are:
...
**Recommended approach**: Treat OnChainUX's design docs as the target architecture, but consider a phased implementation strategy:
...
This would bring OnChainUX to approximately **40-50% feature parity** with Reown AppKit, while retaining its unique infrastructure advantages.
