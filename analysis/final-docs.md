# CinaAuth/OnChainUX vs Reown — Documentation & Developer Experience Final Comparison
...
| Aspect | OnChainUX (CinaAuth) | Reown (WalletConnect) |
|--------|----------------------|------------------------|
...
**Assessment:** OnChainUX has a well-designed document skeleton. The VitePress configuration is clean with proper sidebar grouping, search, and edit links. However, the content volume is a fraction of Reown's.
...
**Reown comparison:** Reown has dedicated pages for each framework (React, Vue, Next.js, Nuxt, Svelte, React Native, iOS, Android, Flutter, Unity), with migration guides from Wagmi/viem and ethers.js. OnChainUX combines all platforms into 3 pages — concise but thin.
...
**Reown comparison:** Reown's API reference is auto-generated from source code (Typedoc/JSDoc) with full type signatures, parameter descriptions, return types, error types, and cross-references. OnChainUX's docs are manually written — good for readability but harder to maintain and likely to drift from source.
...
- ❌ No `@onchainux/swap-sdk` API reference (mentioned in architecture but no docs)
- ❌ No `@onchainux/paymaster` API reference
...
| Platform | OnChainUX Files | Lines | Reown Equivalents |
|----------|-----------------|-------|-------------------|
...
- ❌ No actual OnChainUX SDK Swift source visible
...
### OnChainUX Quick Start Rating: ⭐⭐⭐⭐ (4/5)
...
This is where OnChainUX **significantly outpaces** Reown.
...
**What this means:** AI coding assistants (Claude Code, Cursor, Copilot) working on the OnChainUX repo will have immediate, precise context about the crypto implementation, testing requirements, and project structure. This reduces hallucination risk and improves AI-generated code quality significantly.
...
| Category | OnChainUX | Reown | Notes |
|----------|-----------|-------|-------|
...
| **Security Documentation** | 9/10 | 8/10 | OnChainUX actually better here |
...
**Overall: OnChainUX 6.7/10 vs Reown 7.7/10**
...
### Immediate (Week 1-2)
1. **Set up TypeDoc** for `@onchainux/core`, `@onchainux/react`, `@onchainux/siwe` — auto-generate API reference
...
OnChainUX/CinaAuth documentation has a **strong foundation with excellent structure** and a **unique competitive advantage in AI-agent configuration**. The AGENTS.md and CLAUDE.md files are genuinely ahead of Reown's approach. The security documentation is also notably comprehensive.
...
**The docs are ready for alpha/beta developers but not production teams.** With 2-3 months of focused documentation effort (especially auto-generation, real examples, and English translation), OnChainUX can close the gap significantly.

---

*Report generated by subagent analysis. All assessments based on direct file inspection of the OnChainUX repository.*
