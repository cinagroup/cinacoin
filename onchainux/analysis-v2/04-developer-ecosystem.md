...
> CinaAuth/OnChainUX vs Reown — Developer Experience, Testing Quality, CI/CD, and Ecosystem Tooling.
...
| **Migration guide** | Comprehensive 8-section Reown→OnChainUX migration guide with component/hook mapping tables, breaking changes, automated CLI tool, and checklist |
...
| Migration support | **OnChainUX** ✅ | Dedicated migration guide from Reown — no Reown equivalent |
| Auto-generated API | **Tie** | OnChainUX: TypeDoc → VitePress; Reown: custom tooling |
| Security docs | **OnChainUX** ✅ | Full audit template, crypto review checklist, deployment checklist |
| Bilingual | **OnChainUX** ✅ | Chinese + English |
| Scale | **Reown** ✅ | 776MB of docs vs OnChainUX's compact but complete set |
| Custom domain | **OnChainUX** ✅ | Built into CI/CD pipeline |
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| Platform coverage | **OnChainUX** ✅ | 6 platforms (Web, RN, iOS, Android, Flutter, Unity) vs Reown's 3 |
| Code quality | **Tie** | Both production-quality examples |
| Community validation | **Reown** ✅ | Real GitHub stars (510+130+86) vs OnChainUX's private examples |
| Depth | **OnChainUX** ✅ | Includes Swap, MultiChain, Auth, Deep Linking, Push Notifications |
| Real integrations | **OnChainUX** ✅ | Examples show real RPC, EIP-6963, CoinGecko |

---

## 3. CI/CD Pipeline Maturity

### OnChainUX CI/CD (7 workflows)
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| Pipeline count | **OnChainUX** ✅ | 7 dedicated workflows vs Reown's distributed setup |
| Canary deployment | **OnChainUX** ✅ | Automated canary with Prometheus checks and auto-rollback |
| Security scanning | **OnChainUX** ✅ | Semgrep + Trivy + cargo-audit + gosec + govulncheck + npm audit + weekly cron |
| Multi-language CI | **OnChainUX** ✅ | Single pipeline orchestrating Rust, Go, TS in parallel |
| License compliance | **OnChainUX** ✅ | Automated GPL/AGPL blocking |
| Container scanning | **OnChainUX** ✅ | Trivy + SARIF → CodeQL for all services |
| Bundle size | **Tie** | Both use size-checking (OnChainUX: preactjs; Reown: dangerfile.ts) |
...
### OnChainUX E2E Tests (Playwright)
...
- Reown's `.maestro/` directory suggests mobile UI testing (Maestro framework) — OnChainUX lacks native mobile UI testing
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| Unit test count | **OnChainUX** ✅ | ~58+ test files across 20 packages |
| Integration tests | **OnChainUX** ✅ | 7 full integration flow tests |
| E2E test coverage | **OnChainUX** ✅ | 7 E2E suites, 5 browser targets, mobile included |
| Mobile UI testing | **Reown** ✅ | Maestro framework for native mobile testing |
| Test framework | **OnChainUX** ✅ | Vitest workspace with per-project env config |
| Coverage reporting | **OnChainUX** ✅ | Unified TS+Rust+Go coverage, uploaded as artifacts |
...
|------|---------|---------|
| **AGENTS.md** | AI agent instructions for OnChainUX project | Project overview, crypto architecture (X25519, ChaCha20-Poly1305, SHA-256 with specific library paths), important constraints (no placeholder crypto, no XOR, no AES-GCM fallback), import conventions (ESM with .js extensions), testing commands per language, file locations |
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| Cryptographic specificity | **OnChainUX** ✅ | Exact algorithms, libraries, parameters, and constraints |
| Multi-language support | **OnChainUX** ✅ | TS + Rust guidance |
| Safety constraints | **OnChainUX** ✅ | Explicit "never do X" rules |
...
|------|---------------|---------|
| **CLI** | `packages/cli/` | Project initialization (`onchainux init`), template scaffolding (web, react, vue, next), package manager selection, dry-run mode, **migration CLI** (`onchainux migrate ./project`) |
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| CLI tooling | **OnChainUX** ✅ | Dedicated CLI with init + migration commands |
| Changesets | **Tie** | Both use @changesets |
| Renovate | **Tie** | Both configured |
| Monorepo tooling | **OnChainUX** ✅ | Turbo + pnpm workspace (24 packages) |
| TypeDoc | **OnChainUX** ✅ | 9 entry points, integrated into docs CI |
...
| Aspect | Winner | Notes |
|--------|--------|-------|
| Multi-language linting | **OnChainUX** ✅ | TS + Rust + Go linting in single CI |
| TypeDoc integration | **OnChainUX** ✅ | Auto-generated API docs in docs site |
| Code style docs | **OnChainUX** ✅ | CONTRIBUTING.md with commit conventions |
| Strictness | **OnChainUX** ✅ | Clippy `-D warnings`, strict TS checks |

---

## 8. Overall Developer Experience Scorecard

| Category | OnChainUX | Reown | Winner |
|----------|-----------|-------|--------|
| Documentation | 9/10 | 9/10 | Tie |
| Examples (coverage) | 9/10 | 7/10 | **OnChainUX** |
| Examples (validation) | 6/10 | 9/10 | **Reown** |
| CI/CD maturity | 10/10 | 7/10 | **OnChainUX** |
| Test coverage (quantity) | 9/10 | 7/10 | **OnChainUX** |
| Test coverage (real-world) | 7/10 | 9/10 | **Reown** |
| E2E testing | 9/10 | 7/10 | **OnChainUX** |
| AI agent config | 9/10 | 6/10 | **OnChainUX** |
| CLI tooling | 9/10 | 6/10 | **OnChainUX** |
| Package quality | 9/10 | 7/10 | **OnChainUX** |
| **Overall** | **~8.6/10** | **~7.6/10** | **OnChainUX** |

---

## Key Takeaways

### OnChainUX Advantages
...
4. **Migration CLI needs validation**: The `onchainux migrate` command needs real-world testing against actual Reown projects
