...
**Date:** 2026-05-17  
**Comparison:** CinaAuth/OnChainUX (34 packages) vs Reown AppKit (20 packages + 8 adapters)  
...
## 2. CinaAuth/OnChainUX Full Package Inventory

### JS/TS Client SDK Packages (24)

| Package | NPM Name | LOC (src) | Tests (LOC) | Total Files | Purpose |
|---------|----------|-----------|-------------|-------------|---------|
| `core-sdk` | `@onchainux/core` | 8,605 | 7,312 | 67+30 | Chain-agnostic core, adapters (11 chains), connector, session, store, events, EIP-6963, deep linking, transports |
| `core-ui` | `@onchainux/core-ui` | 3,427 | 1,867 | 31+10 | Web Components (connect button, modal, chain switcher, wallet list, toast), i18n (10 locales), design system, performance |
| `react` | `@onchainux/react` | 620 | 276 | 7+1 | React bindings — Provider, hooks (useAccount, useChainId, useConnect, useDisconnect), wrapper components |
| `react-native` | `@onchainux/react-native` | 2,314 | 615 | 9+3 | React Native — native components, QRScanner, WalletConnect provider |
| `vue` | `@onchainux/vue` | 321 | 424 | 7+3 | Vue 3 bindings — Provider, composables, components |
| `walletconnect-v2` | `@onchainux/walletconnect-v2` | 3,367 | 1,933 | 15+6 | Full WCv2 implementation — relay, crypto (envelope encoding), pairing, session, methods, wallet registry |
| `siwe` | `@onchainux/siwe` | 780 | 928 | 9+3 | Sign-In with Ethereum — message generation, validation |
| `siwx` | `@onchainux/siwx` | 764 | 444 | 10+4 | Sign-In with X — chain-agnostic (EVM, Solana, Bitcoin) |
| `social-login` | `@onchainux/social-login` | 1,131 | 1,270 | 13+6 | OAuth2 social login — Google, Apple, Twitter/X, email + HD wallet derivation |
| `swap-sdk` | `@onchainux/swap-sdk` | 1,213 | 729 | 11+3 | Swap aggregator — Uniswap, 1inch, 0x executors, slippage protection |
| `onramp-sdk` | `@onchainux/onramp-sdk` | 1,064 | 766 | 12+5 | On-ramp aggregator — MoonPay, Ramp, Transak providers, widget |
| `session-keys` | `@onchainux/session-keys` | 1,793 | 1,039 | 10+3 | ERC-4337 session keys — policies, batch, social recovery |
| `aa-sdk` | `@onchainux/aa-sdk` | 486 | 213 | 7+1 | Account Abstraction — SmartAccount, factory, paymaster, bundler client |
| `analytics` | `@onchainux/analytics` | 885 | 114 | 8+1 | GDPR-compliant analytics — event tracking, metrics, consent management |
| `batch-transaction` | `@onchainux/batch-transaction` | 481 | 202 | 10+2 | Batch transactions — atomic ops (transfer, approve, swap, custom) |
| `gas-estimator` | `@onchainux/gas-estimator` | 409 | 132 | 7+1 | Gas estimation for EVM and Solana |
| `token-list` | `@onchainux/token-list` | 569 | 181 | 8+1 | Token discovery — CoinGecko, TrustWallet sources, LRU cache |
| `cross-chain-sync` | `@onchainux/cross-chain-sync` | 735 | 327 | 10+2 | Cross-chain state sync — EVM/Solana/BTC identity linking |
| `passkey-auth` | `@onchainux/passkey-auth` | 645 | 126 | 7+1 | WebAuthn passkey authentication |
| `wallet-recommender` | `@onchainux/wallet-recommender` | 373 | 202 | 4+1 | Wallet recommendation engine with scoring |
| `cli` | `@onchainux/cli` | 628 | 586 | 11+4 | CLI — init, test commands |
| `design-tokens` | `@onchainux/design-tokens` | — | — | 4+2 | Design tokens — default/light/minimal themes, CSS variables |
| `ens-resolver` | `@onchainux/ens-resolver` | ~100 | 0 | 2+0 | ENS contract resolver |
...
|---------|----------|-----|-------|-------|---------|
| `android-kotlin` | Kotlin | ~3,081 | 13 | 7 | Android SDK — OnChainUX core, WC client, UI, deeplinks, push |
| `ios-swift` | Swift | ~5,537 | 21 | 0 | iOS SDK — OnChainUX core, WC client, UI, deeplinks, push |
| `flutter-dart` | Dart | ~4,397 | 22 | 9 | Flutter SDK — adapters, auth, UI, deep links, push |
| `unity-csharp` | C# | ~3,084 | 20 | 0 | Unity SDK — OnChainUX core, adapters, UI, wallet management |
...
Reown's controllers (AccountController, ConnectionController, etc.) map to CinaAuth's store.ts with OnChainUXState management.
...
| Metric | Reown AppKit | CinaAuth/OnChainUX | Delta |
|--------|-------------|-------------------|-------|
...
#### 🟡 Important Gaps
4. **`codemod`** — Build migration tool for WalletConnect → OnChainUX. Critical for adoption from existing WC users.
