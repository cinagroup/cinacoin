...
> CinaAuth/OnChainUX vs Reown (WalletConnect) Infrastructure  
...
| Dimension | CinaAuth/OnChainUX | Reown | Verdict |
|-----------|-------------------|-------|---------|
...
**Bottom Line:** OnChainUX has a **stronger** infrastructure foundation (monitoring, cost management, IaC quality) than what Reown exposes publicly, but is **missing critical backend services** (push notifications, identity keys, ERC-6492 verification, smart account orchestration) that are core to Reown's value proposition.
...
| Feature | Reown | CinaAuth/OnChainUX | Gap |
|---------|-------|-------------------|-----|
...
OnChainUX only mentions push notifications as a **cost line item** in Phase 5 ($50–200/mo). There is no push server implementation, no device registration flow, no notification routing logic.
...
OnChainUX has no equivalent identity key management. The Phase 3 design mentions "MPC module" for Phase 5 but there is no keys-server or key management infrastructure.
...
**Analysis:** Reown's `yttrium` is a cross-platform smart account library. OnChainUX Phase 3 has a detailed smart account design (Account Abstraction, UserOperations, bundler) but the actual implementation library is not yet built. The deploy/ directory has a `bundler/Dockerfile` but no yttrium-equivalent library.
...
| **P1** | Add bundler to Helm chart (currently missing from onchainux/values.yaml) | 2 days |
...
| IaC Component | Reown | CinaAuth/OnChainUX | Gap |
|---------------|-------|-------------------|-----|
...
| Component | CinaAuth/OnChainUX | Reown | Assessment |
|-----------|-------------------|-------|------------|
...
**Assessment:** OnChainUX has a **massive advantage** in cost transparency and control. The built-in KEDA scalers, spot instance configs, budget alerts, and Prometheus cost rules provide visibility and control that Reown's SaaS model cannot match.
...
OnChainUX relay is a **clean-room implementation** with better observability and cost control. However, it lacks:
...
| 14 | **Bundler missing from Helm** | Add bundler to onchainux Helm chart | 2 days |
...
| Dimension | OnChainUX Score | Reown Score | Notes |
|-----------|:--------------:|:-----------:|-------|
