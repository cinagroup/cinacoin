# FINAL: CinaAuth OnChainUX vs Reown — Mobile SDK Quality Assessment
...
OnChainUX provides a **UI-first, white-label wallet connection toolkit** with native components across iOS, Android, and React Native. It has solid UI scaffolding, consistent theming, and deep-link infrastructure. However, it is **fundamentally a thin UI shell** — the core protocol layer (WalletConnect 2.0 signing, session management, relay communication) is stubbed out with simulated connections. Reown's SDKs implement the full WalletConnect protocol stack natively.

**Verdict:** OnChainUX is a well-architected UI layer ready for protocol integration. It is approximately **20-30% of the feature depth** of Reown's corresponding SDKs in terms of production-ready functionality.

---

## 1. iOS Swift SDK

### OnChainUX (`packages/ios-swift/`)

**Files & Structure (11 files):**
```
Sources/OnChainUX/
├── OnChainUX.swift            # Core singleton + config + state
...
Tests/OnChainUXTests/
└── OnChainUXTests.swift       # ~25 unit tests
```
...
### OnChainUX (`packages/android-kotlin/`)

**Files & Structure (6 files):**
```
src/main/kotlin/com/onchainux/
├── core/
│   └── OnChainUX.kt           # Core singleton + config + StateFlows
├── wallet/
...
| Feature | OnChainUX | Reown Kotlin |
|---------|-----------|--------------|
| WalletConnect 2.0 protocol | ❌ Stubbed | ✅ Full implementation |
...
### OnChainUX (`packages/react-native/`)

**Files & Structure (4 source files):**
```
src/
├── index.ts                  # Public exports
├── OnChainUXProvider.tsx     # Context provider + state
...
| Feature | OnChainUX | Reown React Native |
|---------|-----------|---------------------|
...
- Clean React context pattern with `useOnChainUXContext` hook (throws if misused)
...
| Area | OnChainUX | Reown React Native |
|------|-----------|---------------------|
...
| Capability | OnChainUX iOS | OnChainUX Android | OnChainUX RN | Reown (all) |
|---|---|---|---|---|
| Wallet list UI | ✅ | ✅ | ✅ | ✅ |
...
1. **No WalletConnect 2.0 protocol implementation** — The single most critical gap. OnChainUX has no actual WC SDK dependency. All connections are simulated.
...
- `EVMChainAdapter` and `SolanaChainAdapter` are not exposed through the main `OnChainUX` API
...
## 6. Strengths of OnChainUX

Despite the gaps, OnChainUX has genuine strengths:
...
| Dimension | OnChainUX iOS | OnChainUX Android | OnChainUX RN | Reown Swift | Reown Kotlin | Reown RN |
|---|---|---|---|---|---|---|
...
*Report generated from source analysis of OnChainUX mobile packages and comparative knowledge of Reown SDKs.*
