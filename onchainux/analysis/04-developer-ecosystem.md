# Developer Experience, Ecosystem & Quality — CinaAuth/OnChainUX vs Reown Gap Analysis
...
OnChainUX has **six well-structured architecture design documents** (Master + Phase 1-5) covering infrastructure, UI, smart accounts, deployment, and optimization. The code skeleton spans 15 packages across Rust, Go, TypeScript, Solidity, and Vue.
...
| Dimension | CinaAuth/OnChainUX | Reown | Gap |
|-----------|:---:|:---:|:---:|
...
|----------|--------|--------|---------|
| **P0** | Create a hosted documentation site | 2-4 weeks | Use VitePress, Docusaurus, or Nextra. Deploy to docs.onchainux.com. Convert existing Phase docs into developer-facing guides with copy-paste examples. |
...
| **P1** | Add migration guide from AppKit | 1 week | Document API mapping: `createAppKit()` → `OnChainUX.init()`, `wagmi` hooks → `@onchainux/react` hooks. |
...
| **P0** | Create quick-start template | 1 week | `npm create onchainux@latest` that scaffolds a Next.js app with OnChainUX pre-configured. |
...
| **P2** | Create framework-specific examples | Ongoing | Separate repos: `onchainux-nextjs-example`, `onchainux-vite-example`, `onchainux-vue-example`. |
...
### Gap Assessment
| File | Reown appkit | OnChainUX | Gap |
|------|:---:|:---:|:---:|
...
| **P1** | Add bundle size action | 2 days | Track @onchainux/core, @onchainux/ui bundle sizes in PRs. Set budgets: core < 25KB gzipped, ui < 15KB gzipped. |
...
- Phase 3 design doc lists third-party contracts (EntryPoint, Safe, Kernel) with audit status — but OnChainUX's own code has zero audits

### Gap Assessment
| Practice | Reown | OnChainUX | Gap |
|----------|:---:|:---:|:---:|
...
- **No mapping documentation** between AppKit and OnChainUX APIs
...
| Priority | Action | Effort | Details |
|----------|--------|--------|---------|
| **P0** | Create API migration guide | 1 week | Document every AppKit API → OnChainUX equivalent. Example: `createAppKit()` → `OnChainUX.init()`, `useAppKitAccount()` → `useOnChainUX()`. |
| **P1** | Build AppKit compatibility shim | 2-3 weeks | Create `@onchainux/appkit-compat` that provides AppKit-compatible API surface backed by OnChainUX. Allows gradual migration. |
...
| **P2** | Create "side-by-side" comparison dApp | 1 week | A dApp showing identical functionality implemented with both AppKit and OnChainUX. |
...
| 8 | Create quick-start template | 1 week | `npm create onchainux@latest` |
| 9 | Commission smart contract audit | 4-8 weeks | On-chain code requires audit |
| 10 | Add API migration guide | 1 week | Enable AppKit → OnChainUX migration |
...
OnChainUX has **strong technical architecture** — the Phase docs are comprehensive, multi-language, and production-oriented. However, it is currently an **internal design repository**, not a **developer platform**.

The gap between design docs and a usable developer platform is enormous. Reown didn't start with docs and examples — they accumulated them over years of public development. OnChainUX can accelerate by:
