# Gap Analysis: CinaAuth/OnChainUX vs Reown — Smart Accounts, Authentication & Security
...
| Paymaster Contracts | Basic client integration | ✅ 3 contracts (OnChainUX, Token, Verifying) | **CinaAuth leads** |
...
### Verdict: ERC-4337
**CinaAuth/OnChainUX is ahead** in infrastructure depth. The bundler, paymaster, and gas oracle are real implementations. Reown's approach is client-side integration with third-party bundlers.

### Key Gaps in CinaAuth
- The bundler's `_extractSender` in `OnChainUXPaymaster.sol` is a **placeholder** that incorrectly derives sender from the hash bytes — this must be fixed before production
...
| P0-5 | **Fix OnChainUXPaymaster _extractSender** | Currently derives sender from hash bytes — this is wrong. Must decode from actual UserOp calldata. Production blocker. | Small (1-2 days) |
...
| Dimension | Reown | CinaAuth/OnChainUX | Winner |
|-----------|-------|--------------------|--------|
...
CinaAuth/OnChainUX has built **superior infrastructure** (bundler, paymasters, session keys, social recovery) but is **missing the user-facing layer** (authentication, identity, developer adapters). Reown has the opposite profile — excellent developer-facing APIs and auth, but relies on third-party infrastructure.
...
*Analysis generated 2026-05-16 | Based on codebase review of onchainux/packages/ and Phase-3-Smart-Account.md*
