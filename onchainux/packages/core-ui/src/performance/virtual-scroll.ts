/**
 * Virtual Scroll for OnChainUX Wallet Lists
 *
...
export function createVirtualScroller(
  config: VirtualScrollConfig,
  onRangeChange?: (range: VirtualScrollResult) => void
) {
  let currentRange: VirtualScrollResult | null = null;

  const handler = (scrollTop: number): VirtualScrollResult => {
  ...
  return {
    ...
    getCurrentRange: (): VirtualScrollResult | null => currentRange,
  };
}
 ...
 * @param config - Virtual scroll configuration.
 */
export function applyVirtualScroll<T>(
