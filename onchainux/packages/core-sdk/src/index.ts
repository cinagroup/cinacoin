...
export async function createAdapter(
  ...
  switch (config.type) {
    case 'viem': {
      ...
      return mod.createViemAdapter(config.client as unknown as import('./adapters/viem.js').ViemClient, config.connector);
    }
    case 'wagmi': {
      const mod = await import('./adapters/wagmi.js');
      return mod.createMultiChainConnector(config.client as unknown as import('./adapters/wagmi.js').WagmiConfig);
    }
    case 'ethers5': {
      const mod = await import('./adapters/ethers5.js');
      return new mod.Ethers5Adapter(config.client as unknown as import('./adapters/ethers5.js').Ethers5Provider);
    }
    case 'ethers6': {
      const mod = await import('./adapters/ethers6.js');
      return new mod.Ethers6Adapter(config.client as unknown as import('./adapters/ethers6.js').Ethers6Provider);
    }
    case 'ton': {
    ...
    default:
      throw new Error(`Unknown adapter type: ${('type' in config ? String(config.type) : '<none>')}`);
  }
}
