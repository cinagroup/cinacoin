...
import 'package:onchainux/src/types.dart';
import 'package:onchainux/src/wallet_manager.dart';
...
void main() {
  ...
  test('Session data is persisted to encrypted storage', () async {
    ...
    when(() => mockStorage.read(key: 'onchainux_encrypted_session'))
        .thenAnswer((_) async => jsonEncode(sessionData));
    when(() => mockStorage.read(key: 'onchainux_session_expiry'))
    ...
    verify(() => mockStorage.read(key: 'onchainux_encrypted_session')).called(1);
    verify(() => mockStorage.read(key: 'onchainux_session_expiry')).called(1);
  ...
  test('Expired session is cleared on restore', () async {
    ...
    verify(() => mockStorage.delete(key: 'onchainux_encrypted_session')).called(1);
  ...
  test('Disconnect clears persisted session from storage', () async {
    ...
    verify(() => mockStorage.delete(key: 'onchainux_encrypted_session')).called(1);
    verify(() => mockStorage.delete(key: 'onchainux_session_expiry')).called(1);
