# Cinacoin vs Reown (WalletConnect) 对比审计报告

> **日期**: 2026-06-03  
> **审计人**: 000  
> **基准**: Cinacoin monorepo (177 commits, 80 packages) vs Reown 公开产品线

---

## 1. 公司/项目背景

| 维度 | Cinacoin | Reown (原 WalletConnect) |
|------|----------|-------------------------|
| 成立 | 2025-2026 | 2018 (WalletConnect), 2024 改名 Reown |
| 定位 | 全链访问基础设施 | 钱包连接协议 |
...
| 功能 | Cinacoin | Reown |
|------|----------|-------|
| WalletConnect v2 | ✅ 自研完整实现 (9,297 行) | ✅ 创始者/协议标准 |
| EIP-6963 钱包发现 | ✅ | ✅ |
...
**Cinacoin 优势**: 8 个非 EVM 链均有**自研适配器**，Reown 主要依赖第三方集成。
**Reown 优势**: EVM 链覆盖更广 (500+ CAIP 命名空间)。
...
**Cinacoin 优势**: Passkey、Social Login、Embedded Wallet、Session Keys 均为自研，Reown 需要依赖第三方（如 Web3Auth、Privy）。
...
**Cinacoin 显著优势**: 完整的交易基础设施（Paymaster、Bundler、Gas 赞助、跨链桥），Reown 仅为钱包连接层。
...
**Cinacoin 优势**: 完整支付+合规模块，Reown 不涉及此层。
...
**Cinacoin 压倒性优势**: 12 个前端框架/平台 vs Reown 的 2 个（React + React Native）。Reown AppKit 仅限 Web。
...
| Notify Server | ✅ 自推通知 | ✅ Reown Notify |
| Push Server | ✅ FCM/APNs 推送 | ✅ |
| Blockchain API | ✅ 2,838 行 | ❌ |
| Analytics | ✅ analytics + analytics-server | ⚠️ 基础 |
| CDN | ✅ | ✅ Cloudflare |
| Explorer | ✅ explorer | ❌ |
| 云仪表盘 | ✅ cloud-dashboard | ✅ Reown Cloud |
| 后端仪表盘 | ✅ backend-dashboard | ❌ |
| 健康状态页 | ✅ health-status | ✅ status.reown.com |

### 2.8 原生移动 SDK

| 功能 | Cinacoin Android | Cinacoin iOS | Reown |
|------|-----------------|--------------|-------|
| WalletConnect v2 | ✅ 完整 | ✅ 完整 | ❌ 无原生 |
| SIWE 签名 | ✅ | ✅ | ❌ |
| 多链切换 | ✅ | ✅ | ❌ |
| 会话恢复 | ✅ | ✅ | ❌ |
| Compose/UIKit | ✅ 8 个组件 | ✅ | ❌ |
...
| 自托管 | ✅ 全部可自托管 | ❌ Relay 必须用 Reown 云 |
| 收入模式 | 未定义 (基础设施) | SaaS 订阅 |
| 数据控制 | ✅ 用户完全控制 | ⚠️ Reown 持有元数据 |

---

## 6. 核心差异总结

### Cinacoin 独有 (Reown 没有)
...
### Reown 独有 (Cinacoin 缺失)
1. **500+ EVM 链 CAIP 覆盖** (Cinacoin 目前 6 条 EVM + 8 非 EVM)
2. **协议标准制定者** (EIP-4361 SIWE 创建者)
3. **品牌认知度** (WalletConnect 是行业事实标准)
4. **Verify/安全评分** 系统
5. **Push 通知网络** (Reown Push 有数百万设备覆盖)

### 需要加强 (Cinacoin 差距)
1. ⚠️ **EVM 链覆盖**: 仅 6 条 → 需要扩展到 100+ CAIP
2. ⚠️ **Verify/安全评分**: 缺失 → 类似 Reown Verify
...
**Cinacoin 不是 "另一个 WalletConnect"**，而是**全链访问层的超集**：

- Reown = 钱包连接协议 + AppKit（2 个平台）
- Cinacoin = 钱包连接 + 链适配器 + 认证 + 交易 + 跨链 + 支付 + 合规 + 12 平台 SDK

**差距**: EVM 链覆盖广度 (500+ vs 6+8) 和品牌生态。

**机会**: Reown 的 500+ 链是 CAIP 自动注册，实际活跃链 <50。Cinacoin 的 14 条链（6 EVM + 8 非 EVM）中非 EVM 部分远超 Reown。
