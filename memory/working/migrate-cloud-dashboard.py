#!/usr/bin/env python3
"""Batch migrate cloud-dashboard hardcoded colors to DESIGN tokens."""
import re
import os
import glob
...
def process_file(filepath):
    ...
    for pattern, replacement in REPLACEMENTS:
        content = re.sub(r'\b' + pattern.lstrip(r'\b') if pattern.startswith(r'\b') else pattern, replacement, content)
    # Simpler approach: just use the pattern directly
    for pattern, replacement in REPLACEMENTS:
...
def main():
    src = '/home/cina/.openclaw/workspace/onux/apps/cloud-dashboard/src'
    count = 0
