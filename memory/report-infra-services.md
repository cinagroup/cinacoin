...
- **Reown 对标**: Reown 的 Relay/RPC 基础设施支持 WebSocket、多 provider fallback、智能路由。此包仅支持简单 HTTP POST 代理，差距显著。
...
- **功能描述**: WalletConnect 桥接消息的 WebSocket/HTTP 中继服务器。支持 topic-based 消息路由、订阅/取消订阅、发布、速率限制、来源验证、空闲超时断开。
...
- **Reown 对标**: Reown Relay 使用 Durable Objects 实现持久 WebSocket 连接和消息排队。此包无持久化能力，客户端断连即丢消息，远未达到生产级。
...
- **Reown 对标**: Reown Notify 支持多通道通知投递、订阅管理、设备令牌管理。此包功能框架存在，但实际投递未实现，差距极大。
...
- **Reown 对标**: Reown Notify 有完整的 Web3Inbox 系统，包括签名验证、DApp 订阅、推送通道。此包仅提供基础 API 框架。
...
- **Reown 对标**: Reown 使用 ClickHouse 进行大规模分析。此包用 D1（SQLite），适合小规模场景。功能链路完整但规模受限。
...
- **Reown 对标**: Reown 的分析 SDK 深度集成到核心钱包流程中。此包独立性好，但 RemoteProvider 未连接到 analytics-server。
...
- **Reown 对标**: Reown 的 Blockchain API 是托管服务，支持数百条链、代币价格、NFT 索引。此包是客户端 SDK，依赖第三方 API（Alchemy/Covalent），功能覆盖不错但缺少价格数据和更广泛的链支持。
...
- **Reown 对标**: Reown 不直接提供交易索引器（依赖第三方）。此包是 Cinacoin 自建能力，功能定位合理。
...
- **实现状态**: **部分实现**。UI 组件（按钮、模态框）用纯 DOM API 渲染，有 XSS 防护（escapeHtml）、主题支持、钱包选择。但连接逻辑是模拟的（`simulateConnection` 返回假地址 `0x1234...5678`）。没有实际的 WalletConnect/Wagmi 集成。
- **Reown 对标**: Reown AppKit 的 CDN script 是完整的钱包连接方案。此包仅实现 UI 框架，缺少实际的钱包连接。
- **代码质量**: **7/10** — 纯 DOM 渲染避免了 React 依赖，XSS 防护到位。但 `connect.ts` 中模拟连接是明显占位符。Rollup 构建配置存在。
- **缺失项**:
  - **实际钱包连接**（无 WalletConnect/Wagmi 集成）
...
- **Reown 对标**: Reown 使用 Atlassian Statuspage 等商业方案。此包是自研替代，功能足够但规模受限。
...
- **Reown 对标**: Reown Cloud Dashboard 是完整的 SaaS 管理面板（项目管理、分析、API key 管理等）。此包仅覆盖服务监控。
