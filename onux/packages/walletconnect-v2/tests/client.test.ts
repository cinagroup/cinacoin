...
import { isValidWcUri, parseWcUri } from '../src/pairing.js';
import { getDefaultRequiredNamespaces } from '../src/methods.js';
...
describe('WalletConnectClient', () => {
  ...
  it('request() throws when no active session', async () => {
    await expect(client.request('eth_accounts', [])).rejects.toThrow(
      'No active session',
    );
  });

  it('request() with object form throws when no session', async () => {
    await expect(client.request({ method: 'eth_accounts', params: [] })).rejects.toThrow(
      'No active session',
    );
  });
  ...
  it('extendSession throws without session', async () => {
    await expect(client.extendSession(Date.now() + 86400)).rejects.toThrow(
      'No active session',
    );
  });

  it('updateSession throws without session', async () => {
    await expect(client.updateSession({})).rejects.toThrow(
      'No active session',
    );
  });

  it('emitSessionEvent throws without session', async () => {
    await expect(client.emitSessionEvent('eip155:1', 'accountsChanged', [])).rejects.toThrow(
      'No active session',
    );
  });
});
