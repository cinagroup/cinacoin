...
describe('WcSessionManager — expanded lifecycle', () => {
    ...
    it('emits session_approved event on approval', () => {
        ...
        expect(handler).toHaveBeenCalledWith(expect.objectContaining({ type: 'session_approved' }));
    });
    it('emits session_delete event on disconnect', () => {
        const handler = vi.fn();
        manager.on('wcEvent', handler);
        manager.emit('wcEvent', { type: 'session_delete', topic: 'abc' });
        expect(handler).toHaveBeenCalledWith(expect.objectContaining({ type: 'session_delete' }));
    });
    ...
    it('request with object form throws without session', async () => {
        await expect(manager.request({ method: 'eth_sendTransaction', params: [] })).rejects.toThrow('No active session');
    });
