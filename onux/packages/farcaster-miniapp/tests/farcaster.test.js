...
import { generateSignInMessage, parseSignInMessage, buildSignInResult, createSessionPayload, validateSignature, generateNonce, } from '../src/FarcasterAuth.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe('FarcasterProvider', () => {
    ...
    describe('connect', () => {
        ...
        it('should throw if not initialized', () => {
            const ctx = createMockContext();
            const provider = new FarcasterProvider({ context: ctx });
            expect(() => provider.connect(MOCK_ADDRESS)).toThrow('Call initialize() before connect()');
        });
    });
    describe('disconnect', () => {
    ...
    describe('events', () => {
        ...
        it('should subscribe and emit walletConnected event', async () => {
            ...
            await provider.initialize();
            let state = null;
            provider.on('walletConnected', (s) => { state = s; });
            provider.connect(MOCK_ADDRESS);
            expect(state?.connected).toBe(true);
        });
...
describe('buildSignInResult', () => {
    ...
    it('should handle missing verified addresses', () => {
        const user = { fid: 1, username: 'user' };
...
//# sourceMappingURL=farcaster.test.js.map