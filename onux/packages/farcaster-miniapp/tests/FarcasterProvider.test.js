...
describe('FarcasterProvider', () => {
    ...
    it('should throw on unsupported chain switch', async () => {
        ...
        await provider.init();
        await expect(provider.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: '0x89' }] })).rejects.toThrow();
    });
    it('should throw on unsupported methods', async () => {
        const provider = new FarcasterProvider({ contextOverride: mockContext });
        await expect(provider.request({ method: 'eth_sign' })).rejects.toThrow('not supported');
    });
    it('should throw when no account for eth_requestAccounts', async () => {
        const provider = new FarcasterProvider({ contextOverride: mockContext });
        await provider.init();
        await expect(provider.request({ method: 'eth_requestAccounts' })).rejects.toThrow('No wallet connected');
    });
...
//# sourceMappingURL=FarcasterProvider.test.js.map