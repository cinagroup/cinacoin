...
export interface HederaConnector {
  ...
  connect(params?: { network?: HederaNetwork }): Promise<HederaConnectionResult>;
   ...
   */
  signTransaction(params: { transaction: string }): Promise<{ signedTransaction: string }>;
   ...
   */
  executeTransaction(params: { transaction: string }): Promise<{ transactionId: string }>;
   ...
   */
  getBalance(accountId?: string): Promise<{ balance: string; unit: 'tinybar' }>;

  /**
   * Transfer HBAR to another account.
   */
  transferHbar(params: HbarTransferParams): Promise<{ transactionId: string }>;

  /**
   * Transfer an HTS token to another account.
   */
  transferToken(params: TokenTransferParams): Promise<{ transactionId: string }>;

  /**
   * Call a Hedera smart contract function.
   */
  contractCall(params: ContractCallParams): Promise<{ transactionId: string }>;
