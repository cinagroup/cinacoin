...
import { walletGetCapabilities, hasCapability, getChainCapabilities, getSupportedChains, filterByCapability, walletSendCalls, sendSingleCall, sendErc20Transfer, sendBatch, walletGetCallsStatus, waitForCallsStatus, allCallsSucceeded, getFailedReceipts, supportsAtomicBatch, buildAtomicBatch, createEthTransferCall, createContractCall, createErc20ApproveCall, createApproveAndSwapCalls, validateBatchConfig, } from '../../src/eip5792/index.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe('walletGetCapabilities', () => {
    ...
    it('should throw if no account is available', async () => {
        ...
        await expect(walletGetCapabilities(client)).rejects.toThrow('wallet_getCapabilities requires an account address');
    });
    ...
    it('should re-throw non-method-not-found errors', async () => {
        ...
        await expect(walletGetCapabilities(client, MOCK_ADDRESS)).rejects.toThrow('Connection refused');
    });
});
...
describe('walletSendCalls', () => {
    ...
    it('should throw when no from address and no client account', async () => {
        const client = createMockClient({ account: undefined });
        await expect(walletSendCalls(client, {
            version: '1.0.0',
            calls: [{ to: MOCK_ADDRESS }],
...
describe('sendSingleCall', () => {
    it('should wrap a single call in walletSendCalls', async () => {
        const client = createMockClient();
        const call = { to: MOCK_ADDRESS, data: '0xa9059cbb' };
...
describe('sendErc20Transfer', () => {
    it('should create ERC-20 transfer call', async () => {
        const client = createMockClient();
        const result = await sendErc20Transfer(client, MOCK_ADDRESS, MOCK_ADDRESS_2, 1000n, MOCK_ADDRESS, '0x1');
        expect(result.id).toBe('batch-123');
        expect(client.request).toHaveBeenCalled();
    });
    it('should accept string amount', async () => {
        const client = createMockClient();
        const result = await sendErc20Transfer(client, MOCK_ADDRESS, MOCK_ADDRESS_2, '0x3e8', MOCK_ADDRESS);
        expect(result.id).toBe('batch-123');
    });
});
describe('sendBatch', () => {
    ...
    it('should throw on empty calls array', async () => {
        const client = createMockClient();
        await expect(sendBatch(client, [], MOCK_ADDRESS)).rejects.toThrow('sendBatch requires at least one call');
    });
});
// ---------------------------------------------------------------------------
...
describe('walletGetCallsStatus', () => {
    ...
    it('should throw when wallet does not support getCallsStatus', async () => {
        ...
        await expect(walletGetCallsStatus(client, 'batch-123')).rejects.toThrow('wallet_getCallsStatus is not supported by this wallet');
    });
});
describe('waitForCallsStatus', () => {
    ...
    it('should timeout if status stays PENDING', async () => {
        ...
        await expect(waitForCallsStatus(client, 'batch-123', {
            intervalMs: 50,
            timeoutMs: 200,
...
describe('buildAtomicBatch', () => {
    ...
    it('should include capabilities when provided', () => {
        const caps = { '0x1': { paymasterService: { supported: true } } };
        const config = {
            chainId: '0x1',
            from: MOCK_ADDRESS,
            calls: [{ to: MOCK_ADDRESS }],
...
describe('createApproveAndSwapCalls', () => {
    it('should create approve + swap calls', () => {
        const swapCall = { to: MOCK_ADDRESS_2, data: '0x' };
        const calls = createApproveAndSwapCalls(MOCK_ADDRESS, MOCK_ADDRESS_2, 1000n, [swapCall]);
...
describe('validateBatchConfig', () => {
    ...
    it('should throw when a call has invalid hex value', () => {
        const config = {
            chainId: '0x1',
            from: MOCK_ADDRESS,
            calls: [{ to: MOCK_ADDRESS, value: 'not-hex' }],
        };
    ...
    it('should throw when a call has invalid hex data', () => {
        const config = {
            chainId: '0x1',
            from: MOCK_ADDRESS,
            calls: [{ to: MOCK_ADDRESS, data: 'not-hex' }],
        };
        expect(() => validateBatchConfig(config)).toThrow('invalid hex data');
    });
});
//# sourceMappingURL=eip5792.test.js.map