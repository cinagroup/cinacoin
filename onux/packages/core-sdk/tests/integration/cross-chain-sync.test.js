...
class CrossChainSyncManager {
    ...
    registerAdapter(namespace, adapter) {
        this.adapters.set(namespace, adapter);
    }
    async createIdentity(userId, namespace) {
        const adapter = this.adapters.get(namespace);
        if (!adapter)
            throw new Error(`No adapter for namespace: ${namespace}`);
        ...
        const exists = existing.identities.some((i) => i.chainNamespace === identity.chainNamespace && i.address === identity.address);
        if (!exists) {
    ...
    async linkIdentities(userId, namespaces) {
    ...
    async removeIdentity(userId, namespace) {
        const identity = this.identities.get(userId);
        if (!identity)
            return false;
...
//# sourceMappingURL=cross-chain-sync.test.js.map