...
class MockConnector extends Connector {
  readonly id = 'mock-siwe';
  readonly name = 'Mock SIWE Wallet';
  readonly icon = '';
  ...
  setAccounts(addrs: string[]) { this._accounts = addrs; }
  setConnectFail(v: boolean) { this._connectShouldFail = v; }

  async connect(_params?: ConnectParams): Promise<ConnectionResult> {
    if (this._connectShouldFail) throw new Error('Connection rejected');
  ...
  async signTransaction(_tx: TransactionRequest): Promise<string> { return '0xsigned'; }
...
function makeSiwe(connector: Connector, overrides = {}) {
