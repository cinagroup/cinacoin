...
import { TokenList, LRUTokenCache } from '../src/index.js';
const mockTokens = [
    {
...
describe('TokenList', () => {
    ...
    describe('validateToken', () => {
        ...
        it('should reject token with invalid decimals', () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors).toContain('Decimals must be between 0 and 255');
        });
    });
    describe('getToken', () => {
        it('should retrieve a token by address and chainId after fetch', async () => {
            ...
            const list = new TokenList({ sources: [source] });
            await list.fetchAll();
            const token = list.getToken('0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', 1);
...
//# sourceMappingURL=tokenList.test.js.map