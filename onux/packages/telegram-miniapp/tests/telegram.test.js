...
import { validateInitData, parseInitData, buildLoginResult, telegramIdToAddress, generateSignInMessage, isInitDataExpired, } from '../src/TelegramAuth.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function createMockWebApp(overrides = {}) {
    const mainButtonCallbacks = [];
    return {
        ...
        MainButton: {
            ...
            showProgress: vi.fn(),
            hideProgress: vi.fn(),
            onClick: vi.fn((cb) => { mainButtonCallbacks.push(cb); }),
            offClick: vi.fn(),
            onTap: vi.fn(),
            offTap: vi.fn(),
        },
...
describe('TelegramProvider', () => {
    ...
    describe('events', () => {
        it('should subscribe and emit events', async () => {
            ...
            await provider.initialize();
            let emitted = null;
            provider.on('connected', (state) => { emitted = state; });
            provider.connect('0x1234');
...
describe('validateInitData', () => {
    ...
    it('should return false for invalid hash', () => {
        const result = validateInitData('query_id=test&auth_date=1700000000&hash=invalid', 'bot-token');
        expect(result).toBe(false);
    });
});
