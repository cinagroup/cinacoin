...
import { createSignInMessage, verifySignIn, SIWXRegistry, } from '../src/siwx.js';
...
describe('createSignInMessage', () => {
    ...
    it('should throw for unsupported chain type', () => {
        const params = createParams();
        expect(() => createSignInMessage(params, 'unknown')).toThrow('Unsupported chain type');
    });
    it('should enrich with nonce and issuedAt when not provided', () => {
        const params = createParams({ nonce: undefined });
        const message = createSignInMessage(params, 'evm');
...
describe('verifySignIn', () => {
    it('should throw for EVM without provider', async () => {
        ...
        await expect(verifySignIn(input)).rejects.toThrow('EVM verification requires a provider');
    });
    it('should throw for unsupported chain type', async () => {
        ...
        await expect(verifySignIn(input)).rejects.toThrow('Unsupported chain type for verification');
    });
});
...
//# sourceMappingURL=siwx.test.js.map